...
$lnRR_{Q_{10}}$, $lnCVR_{Q_{10}}$ and $lnVR_{Q_{10}}$, as with $Q_{10}$ more generally, all assume that the effect of temperature on physiological rates is linear on the log scale. While $Q_{10}$ has been used exstensively in the physiological literature, there is a preference for using a Boltzmann – Arrhenius relationship to model temperature effects because it has been suggested to better incorperate the kinetics of chemical reactions [@gillooly2001effects]. Over biologically realistic temperature ranges the Boltzmann – Arrhenius relationship and van't Hoff approaches can vary by as much as 15%. Despite this, @white2012information compared modelling the temperature-dependent effects on metabolic rate using both the Boltzmann – Arrhenius relationship and van't Hoff ($Q_{10}$) relationship, showing that, when prokaryotes are excluded, the van't Hoff equation provides a 5.8 fold better fit to the data than the Boltzmann – Arrhenius relationship. Given that we restricted studies only to those that manipulated temperatures within normal operating ranges for a species and we only focus on eukaryotes, $Q_{10}$ should remain an appropriate way to capcture temperature dependent effects. However, given that it is not possible to collect full thermal performance curves for species within our data it was not strickly possible to validate this assumption directly. As such, we conducted a series of additonal analyses to better understand when the log-linearity assumption may be violated.

First, we took the maximum temperature of the experimental manipulations and include this temperature in the models. If log-linearity is a problem, $lnRR_{Q_{10}}$ should be underestimated for studies applying higher temperature treatments because such temperatures are expected to either converge on or cross the thermal optima. The benefit of such an approach means that we are able to statistically correct for the potential non-linearity in the data.

Second, we explored the lowest 25% of $lnRR_{Q_{10}}$ values to understand if these studies have high relative tempereatures compared to the Topt for the species or if they have abnormally high temps. Lab and wild can be used.
...
if(rerun){
  ...
  model1.1_bayes_nl <- brms::brm(effect_size | se(sqrt(V), sigma = TRUE) ~  max_temp_c + (~1 | record_num) + (1 | gr(species_full, cov = A)) + (1 + mid_temp_c| spp) + (1|trait), data2 = list(A=A), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 8, data = lnRRQ10_data)
