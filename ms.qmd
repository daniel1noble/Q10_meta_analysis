...
our own searches that followed the same search protocol. We extracted data from an extra
...
increase in the number of published articles). For full
...
predicted that, if $lnRR_{Q_{10}}$ and $lnVR_{Q_{10}}$ were not
...
acclimation $lnRR_{Q_{10}}$ and $lnVR_{Q_{10}}$ effects varied by
...
$lnVR_{Q_{10_{acute}}}$ would imply that changes in between individual
...
$lnVR_{Q_{10}}$ differed between whole-organism versus biochemical
...
and $lnVR_{Q_{10}}$ were predicted by a populations climate variability
...
adjustments to physiological rates. We used all $lnVR_{Q_{10}}$ data
because there were no differences between $lnVR_{Q_{10_{acclimation}}}$
and $lnVR_{Q_{10_{acute}}}$ and we had no *a priori* reason to believe
climate would impact acclimation and acute $lnVR_{Q_{10}}$ differently.
...
any change in $lnVR_{Q_{10}}$ across latitude and longitude could vary
...
expected change in $lnVR_{Q_{10}}$ for each wild population in our
dataset at the specific populations latitude and longitude. To do this,
we first converted the predicted $lnVR_{Q_{10}}$ to a 1°C change as
...
############
  ...
  # Contrats adult vs juv across habitats regardless of the acute/acclim
...
To better understand the characteristics of the performance curves in a sample that would lead to observed changes in variance across temperature we conducted a simple simulation. To simulate performance curves, we used an asymmetrical Gaussian function:

$$
P_{T} = 2\epsilon^{-\frac{(T-\delta)^2}{2\sigma^2}}\Phi\left(\alpha\frac{T-\delta}{\sigma}\right)
$$ {#eq-sim}

where $T$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where performance is maximized), $\sigma$ the performance breadth, and $\alpha$ the skewness of the performance function or rate variation (see @fig-sim in *Supplementary Materials* for example curves).  While we could have modelled performance curves differently than @eq-sim (e.g., Arrhenius), our intentions were not to model the exact curves (which are not know) but rather identify what might vary across the major habitat types that could contribute to the different patterns of variance change at higher temperatures we observed across habitats.

We simulated *n* = 1000 individual performance curves by varying the amount of between individual variance on each of the key parameters ($\delta$, $\sigma$, $\alpha$) in all possible combinations from 0.01 to 2. We then took the mean and standard deviation of the performance at two temperatures 10 degrees apart (18 and 28 degrees) to calculate the log variance ratio $lnVR_{Q_{10}}$. Our simulations assume that the value of variance does not change depending on the temperatures choosen, but given our analysis restricts the data to temperatures within the normal thermal range of each species we do not envisage this being problematic.
...
`r quantile(change_t, c(0.025, 0.975))[2]`, [@fig-2]A). All results are
...
freshwater ectotherms ([@fig-btrait]C). Unsurprisingly, neither
...
ggsave(figbtrait, filename = "./output/figures/fig5.png", width = 28, height = 14, device = "png")

figbtrait <- magick::image_read("./output/figures/fig5.png")
figbtrait
```
...
ggsave(fig4, filename = "./output/figures/fig6.pdf", width = 28, height = 14, device = "pdf")

fig4 <- magick::image_read("./output/figures/fig6.pdf")
fig4
```
...
increase with increasing temperature across all habitat types ([@fig-2]B) with less heterogeneity overall ($I^2_{sv}$ = `r mean(tot_i2_VR)`, 95% CI:
`r quantile(tot_i2_VR, 0.025)` to `r quantile(tot_i2_VR, 0.975)`; "Model 1"; see *Supplementary Materials* @fig-s3). However, physiological rate variance increased significantly more in freshwater for acute resposnes (freshwater vs terrestrial: $\beta$ = `r mean(f_m_c_a)`, 95% CI: `r quantile(f_m_c_a, 0.025)` to `r quantile(f_m_c_a, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(f_m_c_a))`), but not for acclimation responses because increases in rates were dampended by acclimation resulting in smaller increases in variance (freshwater vs terrestrial: $\beta$ = `r mean(f_m_c_aclim)`, 95% CI: `r quantile(f_m_c_aclim, 0.025)` to `r quantile(f_m_c_aclim, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(f_m_c_aclim))`). While marine ectotherms had larger increases in physiological rate variance compared to terrestrial ectoterms these were not signigicant (Acute: $\beta$ = `r mean(m_t_c_a)`, 95% CI: `r quantile(m_t_c_a, 0.025)` to `r quantile(m_t_c_a, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(m_t_c_a))`; Acclimation: $\beta$ = `r mean(m_t_c_aclim)`, 95% CI: `r quantile(m_t_c_aclim, 0.025)` to `r quantile(m_t_c_aclim, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(m_t_c_aclim))`)([@fig-2]B). Marine and freshwater did not differ in how much variance changed at higher temperatures (Acute: $\beta$ = `r mean(f_m_c_a)`, 95% CI: `r quantile(f_m_c_a, 0.025)` to `r quantile(f_m_c_a, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(f_m_c_a))`; Acclimation: $\beta$ = `r mean(f_m_c_aclim)`, 95% CI: `r quantile(f_m_c_aclim, 0.025)` to `r quantile(f_m_c_aclim, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(f_m_c_aclim))`). There were no differences between
$lnVR_{Q_{10_{acute}}}$ and $lnVR_{Q_{10_{acclimation}}}$ in any habitat ([@fig-2]B). Differences
in variance between habitats were driven by different scaling relationships between mean and variance with slopes varying across habitat types (see *Supplementary
Materials* analyses with $lnCVR_{Q_{10}}$; @tbl-slopes & @fig-mv). 

Overall, there was a `r mean(t_VR_mean_ef)`%
(95% CI: `r quantile(t_VR_mean_ef, 0.025)[1]` to
`r quantile(t_VR_mean_ef, 0.975)[1]`, $p_{MCMC}$ =
`r pmcmc(t_VR_mean)`) increase in physiological rate variance
...
`r pmcmc(m_VR_mean)`) increase in variation for marine
ectotherms when temperatures increased by 10$^\circ$C. In contrast,
freshwater ectotherms exhibited a `r mean(f_VR_mean_ef)`%, 95% CI: `r quantile(f_VR_mean_ef, 0.025)[1]`
to `r quantile(f_VR_mean_ef, 0.975)[1]`, $p_{MCMC}$ =
`r pmcmc(f_VR_mean)` increase in variance at high temperatures
(@fig-2). (results from "Model 2"). The relative differences between habitats were similar when considering $lnCVR_{Q_{10}}$ (See *Supplementary Materials*).

$lnVR_{Q_{10}}$ values from our simulations matched our empirical results quite nicely in key areas of parameter space (@fig-sim2). Interestingly, for a given between individual variance in thermal breadth, terrestrial ectotherms are predicted to have lower levels of between individual variation in thermal optima compared to marine and freshwater ectotherms ([@fig-sim2]). In contrast, terrestrial ectotherms are expected to have higher levels of between individual variation in thermal breadth when controlling for between individual variation in thermal optima ([@fig-sim2])


```{r, fig-sim2}
#| label: fig-sim2
#| fig-align: center
#| fig-cap: "Expected $lnVR_{Q_{10}}$ when varying individual variation in thermal breadth($\\sigma_{_b_{\\delta}}$ = {0.01, 2}) and  thermal optima ($\\sigma_{_b_{\\sigma}}$ = {0.01, 2}) while fixing the rate variance constant ($\\sigma_{_b_{\\alpha}}$ = 0.01). In all simulations, $\\delta$ = 35, $\\sigma$ = 9, $\\alpha$ = -15, and *n* = 1000 individuals were simulated for each combination of $\\sigma_{_b_{\\delta}}$ and $\\sigma_{_b_{\\sigma}}$. The parameter space that matches the observed mean $lnVR_{Q_{10}}$ for terrestrial (green), marine (orange) and freshwater (blue) ectotherms is also shown. Dashed lines indicate the relative differences between the three habitat types when holding one variance parameter constant."
...
responses ($lnRR_{Q_{10}}$) varied across habitats while
variance ($lnVR_{Q_{10}}$) in physiological rates of ectotherms
increased, with the extent of this increase varying across habitats. Our results uncover an hitherto unrecognised dynamic
where the limited benefits of acclimation are offset by increases in trait
variance that may have impacts on how ectotherm populations across these different habitats
will be able to adapt to increased temperatures.

### ***Acclimation capacities vary among habitats but are often still limited***

We show that acclimation of physiological rates and changes in their
...
homeostasis [@Gunderson2015-hz; @morley2019physiological]. Importantly, even though marine and freshwater ectotherms were
capable of partial acclimation on average, the effect size was small (amounting to $Q_{10}$ dropping
from ~1.8 to 1.6), suggesting acclimation will still provide limited opportunity for aquatic ectotherms when coping with higher temperatures.
...
### ***Increased variability in physiological rates across habitats: insights into the adaptive potential of physiological processes in the face of climate change?***
In contrast to acclimation capacity, variance in physiological rates increased across habitats with effect sizes being 3-5 times larger than those observed for acclimation. Mechanistically, it is unclear what is contributing to the increased variation in physiological rates at higher temperatures, but it could be the result of increased among-individual variability in how biochemical, cellular and physiologcial processes function at higher temperatures to maintain homeostasis [@tattersall2012coping; @angilletta2009thermal; @somero1995proteins;
@fields2001protein]. Higher temperatures increase membrane fluidity affecting electrochemical gradients and impact protein structure and function [@somero1995proteins;
@fields2001protein; @tattersall2012coping]. Such challenges (among others) may increase existing variation among indiviudals within a population. Some support for these mechanisms exist given that variation in biochemical traits did show larger increases in variability at high teperatures compared to whole-organism traits, although this was only significant for marine organisms. Increases in variance are probably dependent on the type of biochemical reaction being measured, and so, the differences across habitats could be attributable to the types of traits being measured. 

Importantly, increased variance in physiological rates was not equal among ectotherms accupying terrestrial, marine and freshwater habitats. For example, increases in variance were particularly
pronounced in freshwater ectotherms (~60% increase over 10 degrees Celsius) compared to terrestrial ectotherms (~36% increase over 10 degrees Celcius). The changes in variability we observed across habiats can be explained by differences in among individual variation in thermal breath and optima of individual performance curves between habitats. Consistent differences in thermal performance curves across these major habitat types have never been formally compared, probably because of the challeges in measuring full performance curves across taxa. However, a multitude of studies support the general notion that species occupying different habiats (terrestrial vs aquatic) should differ. @p

Understanding the relative role that plasticity and genetic adaptation will play in response to future climates has attracted considerable attention [@urban2023and]. If the increased variability we observed in our study is underpinned by genetic variation then our results suggest evolutionary adaptation will be more important than acclimation as populations experience future climates. Increased expression of genetic variation in response to novel environments has been shown by numerous studies but primarily for morphological and life-history traits [noble rowinsku rogel, ]. Our results further highlight the vulnerability of terrestrial
ectotherms to climate change. Assuming changes in
...
physiological variance could limit adaptation in terrestrial habitats more than aquatic habiats in the 
future [@urban2023and]. For example, under climate we expect an increase of only ~13% in terrestrial habitats whereas for freshwater habiats we expect variation in physiological rates to increase by 29%.

------
Only in freshwater ectotherms did acclimation in
physiological rates coincide with no change in relative variation.
Such a mechanism may also help explain the
...
[@angilletta2009thermal].
 ...
 We also expected relative variance to differ
...
that impact enzyme function [@fields2001protein].
 ...
 Of course, considering how behavioural
...
variance estimates. Nonetheless, quantitative measures on the changes in variance in physiological rates could be better
...
It is clear that many fascinating questions remain
...
this metric of thermal predictability in the *Supplemental Materials* below.
...
#| fig-cap: "Mean-standard deviation relationships for the acute and acclimation responses across all habitats. Relationships are depicted for the low temperature treatment of the study (A) and high temperature treatment (B)."
 ...
 p_mv <-  (mv1 / mv2)  + plot_annotation(tag_levels = "A") + theme(plot.tag = element_text(size = 20))
  ggsave(p_mv, file = "./output/figures/figS5.png", height = 8, width = 8)
  
  p_mv <- magick::image_read("./output/figures/figS5.png")
  p_mv
```
...
################## 
  ...
  ggsave(p3, filename = "./output/figures/figS6.pdf", width = 11.5, height = 8)

  figs1 <- image_read("./output/figures/figS6.pdf")
  figs1
```
...
ggsave(figbtrait_vr, filename = "./output/figures/figS7.png", width = 28, height = 8, device = "png")

figbtrait <- magick::image_read("./output/figures/figS7.png")
figbtrait
```
...
ggsave(figcvr_lh, filename = "./output/figures/figS8.pdf", width = 28, height = 10, device = "pdf")

figcvr_lh <- magick::image_read("./output/figures/figS8.pdf")
figcvr_lh
...
Acclimation capacity varied across trait categories and habitat with measures of resting metabolic rate, including associated biochemical reactions like oxidative phosphorylation (OXPHOS) and ATPase activity, acclimating in marine and freshwater ectotherms (@fig-lnrrtrait). Whether relative variation in physiological rates changes also depended on trait type, with freshwater ectotherms generally maintaining variance in physiological rates better than marine and freshwater ectotherms (@fig-vrtrait & @fig-cvrtrait). We note though that some traits have very small sample sizes on their own and should be interpreted with caution.
...
ggsave(fig2, filename = "./output/figures/figS9.png", height = 20, width = 15)

img_fig2 <- magick::image_read("./output/figures/figS9.png")
img_fig2
```
...
ggsave(figvrtrait, filename = "./output/figures/figS10.png", height = 20, width = 15)

img_figvrtrait <- magick::image_read("./output/figures/figS10.png")
img_figvrtrait
```
...
ggsave(figcvrtrait, filename = "./output/figures/figS11.png", height = 20, width = 15)

img_figcvrtrait <- magick::image_read("./output/figures/figS11.png")
img_figcvrtrait
```
...
ggsave(figs3, width = 16, height = 5.7, filename = "./output/figures/figS12.png")

figs3 <- image_read("./output/figures/figS12.png")
figs3
```
...
ggsave(p_acf, width = 18, height = 10, filename = "./output/figures/figS13.pdf")

  p_acf <- image_read("./output/figures/figS13.pdf")
  p_acf 
```
...
ggsave(figs2, width = 11.5, height = 4.8, filename = "./output/figures/figS14.png")

figs2 <- image_read("./output/figures/figS14.png")
figs2
```
...
where $T$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where performance is maximized), $\sigma$ is the performance breadth, and $\alpha$ is the skewness of the performance function or rate variation.  To understand how each parameter impacts the shape of performance curces, we simulated 40 individuals with varying amounts of between individual variation in performance breadth, optima and rate variation. We then calculated the relative variance in performance across the temperature gradient as the variance in performance at each temperature divided by the maximum performance at that temperature. This simple analysis identified thermal optima and breath as being the major factors likely leading to the observed patterns in $lnVR_{Q_{10}}$ we identify in our meta-analysis.
...
ggsave(p_sim, width = 22.5, height = 18, filename = "./output/figures/figS15.png")
 
p_sim <- image_read("./output/figures/figS15.png")
p_sim
```
