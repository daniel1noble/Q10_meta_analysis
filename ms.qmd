...
#| label: fig-phylo
#| echo: false
#| fig-cap: "Phylogenetic distribution of acute and acclimation $Q_{10}$ estimates across major habitats.  The total number of acute and acclimation $Q_{10}$ effect sizes are highlighted as well as whether the taxa are marine, freshwater or terrestrial. Silhouettes are only representative taxa of major clades within the tree.
...
```{r, fig-3}
#| label: fig-3
#| fig-cap:  **A)** Mean acute and acclimation $lnRR_{Q_{10}}$ across marine, freshwater, and terrestrial systems. **C)** Mean acute and acclimation $lnCVR_{Q_{10}}$ across traits for marine, freshwater and terrestrial systems. Note there were no differences between acute and acclimation $Q_{10}$ types. For both plots, k = total number of effect size estimates while the numbers in brackets indicate the number of species. Sample sizes are the same for panel A and B. Thick bars are 95% confidence intervals (CI) and thin bars 95% prediction intervals (PI). $\Beta$ values are the contrasts between acute and acclimation means. $\mu$ values are the overall mean $lnRR_{Q_{10}}$ and $lnCVR_{Q_{10}}$ averaged across acute and acclimdation types within each habitat type. In both cases, their 95% CI's are indicated within square brackets. $p_{MCMC}$ values are the posterior probability of the contrast being different from zero."
#| warning: false
#| echo: false
...
##################
  ...
  p3a <- orchaRd::orchard_plot(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45, legend.pos = "top.left", trunk.size = 1.8, branch.size = 2.8, alpha = 0.4) + scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) +
  theme(legend.direction = "vertical") +
  
  annotate("text", x = 1.5, y = 0, 
         label =TeX(paste0("$\\beta = $", round(model1.2$b[4], 2), "[", round(model1.2$ci.lb[4], 2)," to ", round(model1.2$ci.ub[4], 2), "] ", " $p_{MCMC}$ < 0.001")), hjust = 0, size = 8) + 
  annotate("text", x = 1.20, y = -0.5, 
         label =paste0(round(mean(change_f), 3), "%"), hjust = 0, size = 8) + labs(shape = "Type") +
  
  annotate("text", x = 2.5, y = 0, 
         label =TeX(paste0("$\\beta = $", round(mean(m_acclim - m_acute), 2), "[", round(quantile(m_acclim - m_acute, 0.025), 2)," to ", round(quantile(m_acclim - m_acute, 0.975),2), "] ", " $p_{MCMC}$ = 0.01")), hjust = 0, size = 8) + 
  annotate("text", x = 2.20, y = -0.5, 
         label =paste0(round(mean(change_m), 3), "%"), hjust = 0, size = 8) +
  
  annotate("text", x = 3.5, y = 0, 
         label =TeX(paste0("$\\beta = $", round(mean(t_acclim - t_acute), 2), "[", round(quantile(t_acclim - t_acute, 0.025), 2)," to ", round(quantile(t_acclim - t_acute, 0.975),2), "] ", " $p_{MCMC}$ = 0.36")), hjust = 0, size = 8) + 
  annotate("text", x = 3.20, y = -0.5, 
         label =paste0(round(mean(change_t), 3), "%"), hjust = 0, size = 8) +
...
################## 
  ...
  annotate("text", x = x1-0.15, y = -1.5, label = TeX(paste0("$p_{MCMC} = $", round(pmcmc(f_cvr_mean), digit = 2))), hjust = 0, size = 6) + 
  annotate("text", x = x1-0.15, y = 1, 
           label =paste0(round(mean(exp(f_cvr_mean)-1)*100, 2), "%"), hjust = 0, size = 6) + 
  
  annotate("text", x = x2, y = -1.3, 
  ...
  annotate("text", x = x2-0.15, y = -1.5, label = TeX(paste0("$p_{MCMC} = $", round(pmcmc(m_cvr_mean), digit = 2))), hjust = 0, size = 6) +
  annotate("text", x = x2-0.15, y = 1, 
           label =paste0(round(mean(exp(m_cvr_mean)-1)*100, 2), "%"), hjust = 0, size = 6) + 
  
  annotate("text", x = x3, y = -1, 
  ...
  annotate("text", x = x3-0.15, y = 1, 
    ...
    annotate("text", x = x3-0.15, y = -1.5, label = TeX(paste0("$p_{MCMC} = $", round(pmcmc(t_cvr_mean), digit = 3))), hjust = 0, size = 6) + ylim(-1.5, 1.75) + 
  ...
  fig3 <- (p3a + p3b) + plot_annotation(tag_levels = "A", tag_suffix = ")")
   ...
   ggsave(fig3, filename = "./output/figures/fig3_r1.pdf", width = 9.3, height = 8.8)
...
```{r, fig-cvrlh}
#| label: fig-cvrlh
#| fig-cap: "Estimated mean acclimation and acute $lnRR_{Q_{10}}$ (A-C) and $lnCVR_{Q_{10}}$ (D-F) for adult and juvenile life-history stages for Marine (A & D), Freshwater (B & E) and Terrestrial (C & F) ectotherms. k = total number of effect size estimates while the numbers in brackets indicate the number of species. Thick bars indicate 95% confidence intervals and thin bars indicate 95% prediction intervals. Raw data for both adult and juvenile life-history stages also presented but points are not distinguished by different symbols for ease of presentation."
#| echo: false
#| warning: false
...
p1 <- orchard_plot(model1.4, mod = "type", at = list(c_accl_time = 0, habitat = "m"), group = "record_num", xlab = TeX("$lnRR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, legend.pos = "none", angle = 45, branch.size = branch.size, trunk.size = trunk.size) + labs(title = "Marine") + add_theme() + ylim(-0.8, 1.8)

p2 <- orchard_plot(model1.4, mod = "type", at = list(c_accl_time = 0, habitat = "f"), group = "record_num", xlab = TeX("$lnRR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, branch.size = branch.size, trunk.size = trunk.size, angle = 45,)+ labs(title = "Freshwater")  + add_theme() + theme(legend.position = "none", axis.text.y = element_blank()) + ylim(-0.8, 1.8)

p3 <- orchard_plot(model1.4, mod = "type", at = list(c_accl_time = 0, habitat = "t"), group = "record_num", xlab = TeX("$lnRR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, branch.size = branch.size, trunk.size = trunk.size, angle = 45)+ labs(title = "Terrestrial") + add_theme() + theme(legend.position = "none", axis.text.y = element_blank()) +  ylim(-0.8, 1.8)

p1_cvr <- orchard_plot(model2.4, mod = "type", at = list(c_accl_time = 0, habitat = "m"), group = "record_num", xlab = TeX("$lnCVR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, legend.pos = "none", angle = 45, branch.size = branch.size, trunk.size = trunk.size) + ylim(c(-2, 2.2)) + add_theme() + ylim(-1.5, 1.5) 

p2_cvr <- orchard_plot(model2.4, mod = "type", at = list(c_accl_time = 0, habitat = "f"), group = "record_num", xlab = TeX("$lnCVR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, branch.size = branch.size, trunk.size = trunk.size)  + add_theme() + theme(legend.position = "none", axis.text.y = element_blank()) + ylim(-1.5, 1.5)

p3_cvr <- orchard_plot(model2.4, mod = "type", at = list(c_accl_time = 0, habitat = "t"), group = "record_num", xlab = TeX("$lnCVR_{Q_{10}}$"), condition.lab = "Life Stage", by = "life.hist",  g = FALSE, k = FALSE, branch.size = branch.size, trunk.size = trunk.size)+ add_theme() + theme(legend.position = "none", axis.text.y = element_blank()) + ylim(-1.7, 1.5)

fig4 <-(p1 | p2 | p3) / (p1_cvr | p2_cvr | p3_cvr) + plot_annotation(tag_levels = "A", tag_suffix = ")") + theme(legend.position = "bottom")
...
Thermal variability (i.e., $CV$) and predictability experienced by a population in the past did not explain acclimation capacity or changes in physiological rate variance among terrestrial, marine or freshwater populations (@fig-4; @fig-5). 
...
#### Exploring the impact of temperature on $lnRR_{Q_{10}}$ 
