...
Lastly, to illustrate the effects that climate warming could have on physiological rate variance we also extracted climate projections and calculated thermal variability and predictability for the future. We used the CanESM2 climate model (2005-2100) under a high emissions scenario (RCP8.5). 
...
##################
  ...
  cols <- c("#053061","#D1E5F0",  "#67001F")
  k_lnrr <- manual_extract(model1.2, mod = "habitat", group = "record_num", exclude = NULL)

  p1.2 <- orchaRd::orchard_plot(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45, legend.pos = "top.left", trunk.size = 1.8, branch.size = 2.8, alpha = 0.4) + scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) +
  ...
  scale_fill_manual(values = cols) + scale_color_manual(values = rep("black",3)) + ylim(-0.5, 1.5) 
...
################## 
  ...
  #sum_data_p2 <- mod_results(model2.2, mod = "habitat", group = "record_num", by = "type", at = list(type = c("acute", "acclim")))

  #ggplot(lnCVRQ10_data, aes(y = effect_size, x = habitat, colour = habitat, shape = type)) + ggbeeswarm::geom_quasirandom( method = "tukey", aes(color = habitat)) + 
   #scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) + ylim(-1.5, 1.5) + scale_colour_manual(values = viridis_cols) + 
   # theme_classic() + coord_flip()
  
  p2 <- orchard_plot(model2.2, xlab = TeX("$lnCVR_{Q_{10}}$"), by = "type", at = list(type = c("acute", "acclim")), mod = "habitat", group  = "record_num", weights = "prop", angle = 45, trunk.size = 1.8, branch.size = 2.8, alpha = 0.4, k.pos = "none") +
              scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) + ylim(-1.5, 1.5) +  
  
  scale_fill_manual(values = cols) + scale_color_manual(values = rep("black",3)) +
  ...
  ggsave(p2, filename = "./output/figures/fig2.pdf", width = 9.3, height = 8.8)

  (p1.2 + p2) + plot_annotation(tag_levels = "A", tag_suffix = ")")
...
bubble_plot(model1.2, mod = "max_temp_c", group = "record_num", ylab = TeX("$lnRR_{Q_{10}}"), xlab = TeX("Centered Maximum Temperature ($^{\\circ}$C)"), by = "habitat", condition.nrow = 1)  + theme(axis.title  = element_text(size = 30), 
