---
title: "Reduced plasticity and relative variance in physiological rates of ectotherm populations under climate change"
...
We first fit multi-level meta-analysis (MLMA) models (i.e., intercept-only models) for both $lnRR_{Q_{10}}$ and $lnCVR_{Q_{10}}$, that included study, species, trait type, and phylogeny as random effects to account for non-independence and identify sources of variability. Our MLMA models allowed us to partition the variation in $lnRR_{Q_{10}}$ and $lnCVR_{Q_{10}}$ among these key sources while accounting for total sampling variance in each. This allowed us to calculate the proportion of total heterogeneity [i.e., $I_{total}^2$; *sensu* @Nakagawa2012-oc; @Noble2022-ty] along with various $I^2$ metrics describing the proportion of variance explained by each random effect level [@Nakagawa2012-oc]. We also present 95% prediction intervals which describe the expected distribution of effects from future studies [@Nakagawa2021-ls; @Noble2022-ty]. 
...
In all our models we tested and corrected for the possibility of the log-linearity assumption of our effect sizes being violated. We predcited that, if $lnRR_{Q_{10}}$, $lnCVR_{Q_{10}}$ and $lnVR_{Q_{10}}$ were not strictly log-linear than we should see a decrease in average effect size for studies applying higher temperature treatments because these temperatures are expected to either converge on or cross the thermal optima of the performance curve causing reaction rates to decelerate or decrease beyond $T_{op}$ [@michaletz2024hotter]. The benefit of such an approach means that we could still take advantage of the excellent properties of $Q_{10}$ as an effect size while statistically correcting for the potential non-linearity that would be expected in the data at high temperatures. Given that our data included a wide range of species and habitats, we also included a random slope of maximum temperature that varied across species because we, *a priori*, expected that each species will vary in their thermal performance curve and the chosen temperatures experimentally manipulated along the performance curve. To improve model interpretation we centred the maximum temperature around the mean temperature of the experimental treatments.
...
Overall, $lnRR_{Q_{10_{acclimation}}}$ was `r abs(mean(avg_change))`% lower than  $lnRR_{Q_{10_{acute}}}$ across all habitats (95%CI: `r quantile(avg_change, c(0.025, 0.975))[1]` to `r quantile(avg_change, c(0.025, 0.975))[2]`%). Ectotherms in marine and freshwater environments showed partial compensation of physiological rates ([@fig-2]A) amounting to reduced $lnRR_{Q_{10_{acclimation}}}$ of `r abs(mean(change_f))`% (95% CI: `r quantile(change_f, c(0.025, 0.975))[1]` to `r quantile(change_f, c(0.025, 0.975))[2]`) in freshwater and `r abs(mean(change_m))`% (95% CI: `r quantile(change_m, c(0.025, 0.975))[1]` to `r quantile(change_m, c(0.025, 0.975))[2]`) in marine environments. In contrast, terrestrial ectotherms showed no acclimation (possibly even inverse acclimation) -- showing a `r abs(mean(change_t))`% increase in $lnRR_{Q_{10_{acclimation}}}$ (95% CI: `r quantile(change_t, c(0.025, 0.975))[1]` to `r quantile(change_t, c(0.025, 0.975))[2]`, [@fig-2]B). 

Nonetheless, effect heterogeneity was high (only `r mean(tot_i2)`% of the variance was the result of sampling variability, 95% CI: `r quantile(tot_i2, 0.025)[1]` to `r quantile(tot_i2, 0.975)[1]`%), and most variance was explained by the specific study and type of trait (Study: `r mean(stdy_i2)`% , 95% CI: `r quantile(stdy_i2, 0.025)[1]` to `r quantile(stdy_i2, 0.975)[1]`%; Trait Type: `r mean(trait_i2)`% , 95% CI: `r quantile(trait_i2, 0.025)[1]` to `r quantile(trait_i2, 0.975)[1]`%). Evolutionary relationships among taxa and species ecology (i.e., species random effect) explained little variation in acute and acclimation responses (Species: `r mean(spp_i2)`% , 95% CI: `r quantile(spp_i2, 0.025)[1]` to `r quantile(spp_i2, 0.975)[1]`%; Phylogeny: `r mean(phylo_i2)`% , 95% CI: `r quantile(phylo_i2, 0.025)[1]` to `r quantile(phylo_i2, 0.975)[1]`%).
...
# Look at the environment details across different habitats. Seems very similar.
...
p_cv <- (p_t_rr + p_m_rr + p_f_rr) / (p_t_cv + p_m_cv + p_f_cv) + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 36))

ggsave(p_cv, width = 18, height = 10, filename = "./output/figures/fig_p_cv_r1.pdf")
...
col <- c("#0871B9", "#F3B40D", "#1BB908")
ggplot(data = lnRRQ10_data, aes(x = temp_2, y = effect_size)) + 
...
ggplot2::geom_smooth(data = mod_table, ggplot2::aes(x = moderator, y = estimate), method = "loess", 
            ...
            scale_fill_manual(labels = c("f" = "Freshwater", "m" = "Marine", "t" = "Terrestrial"), values = col)+ 
...
```{r, fig-mv}
  ...
  (mv1 / mv2)  + plot_annotation(tag_levels = "A", tag_suffix = ")") + theme(plot.tag = element_text(size = 20, face = "bold"))

```

Overall, we found that the relationship between the mean and SD of the acute and acclimation responses was generally linear, suggesting that the use of $lnCVR_{Q_{10}}$ was justified (@fig-mv). Overall, the scaling relationship between log(SD) and log(mean) was sub-linear across all habitats (Freshwater = $\Beta$ = `r fixef(model_mv1)[2]`, 95% CI: `r fixef(model_mv1)[2,3]` to  `r fixef(model_mv1)[2,4]`; Marine = $\Beta$ = `r fixef(model_mv1)[2] + fixef(model_mv1)[5]`, 95% CI: `r fixef(model_mv1)[2,3] + fixef(model_mv1)[5,3]` to `r fixef(model_mv1)[2,4] + fixef(model_mv1)[5,4]`; Terrestrial: $\Beta$ = `r fixef(model_mv1)[2] + fixef(model_mv1)[6]`, 95% CI: `r fixef(model_mv1)[2,3] + fixef(model_mv1)[6,3]` to `r fixef(model_mv1)[2,4] + fixef(model_mv1)[6,4]`) suggesting that the relative variance in physiological rates does not increase proportionally as the mean increases. Importantly, the scaling relationship between log(SD) and log(mean) differed significantly between habitats (Freshwater compared to Marine: Contrast = `r fixef(model_mv1)[5]`, 95% CI: `r fixef(model_mv1)[5,3]` to `r fixef(model_mv1)[5,4]` and Terrestrial to Marine: Contrast = `r fixef(model_mv1)[6]`, 95% CI: `r fixef(model_mv1)[6,3]` to `r fixef(model_mv1)[6,4]`).
...
################## 
  ...
  col <- c("#0871B9", "#F3B40D", "#1BB908")
  p3 <- orchard_plot(model3.2, xlab = TeX("$lnVR_{Q_{10}}$"), mod = "habitat", by = "type", group  = "record_num", weights = "prop", angle = 45, trunk.size = 1) +
  ...
  annotate("text", x = 1.20, y = -2, label =paste0(round(mean(f_VR_mean_ef), 3), "%"), hjust = 0, size = 8) + labs(shape = "Type") +
  ...
  annotate("text", x = 2.20, y = -2, label =paste0(round(mean(m_VR_mean_ef), 3), "%"), hjust = 0, size = 8) + labs(shape = "Type") +
  ...
  annotate("text", x = 3.20, y = -2, label =paste0(round(mean(t_VR_mean_ef), 3), "%"), hjust = 0, size = 8) + labs(shape = "Type") + scale_fill_manual(values = c(col))
...
With these limitations in mind, we used our temperature time series to calculate autoregressive correlation in temperaure across the entire time series. We then modelled how this measure of thermal predictability was related to plasticity. 
...
ggsave(p_acf, width = 18, height = 10, filename = "./output/figures/fig_p_acf_r1.pdf")
  ...
  p_acf <- image_read("./output/figures/fig_p_acf_r1.png")
  p_acf 
```
...
# USe effective sample size instead because sampling variance estimates could indue a spurious correlation with our effect size
    lnRRQ10_data <- lnRRQ10_data %>%  
              mutate(n_ef = ifelse(name == "lnRR_Q10_acute_t1", (4*r1.1_N*r1.2_N)/(r1.1_N + r1.2_N), # acute 1
                            ifelse(name == "lnRR_Q10_acute_t2", (4*r2.1_N*r2.2_N)/(r2.1_N + r2.2_N), # acute 2
                                                                (4*r1.1_N*r2.2_N)/(r1.1_N + r2.2_N)))) # acclimation
  
    lnCVRQ10_data <- lnCVRQ10_data %>%  
              mutate(n_ef = ifelse(name == "lnCVR_Q10_acute_t1", (4*r1.1_N*r1.2_N)/(r1.1_N + r1.2_N), # acute 1
                            ifelse(name == "lnCVR_Q10_acute_t2", (4*r2.1_N*r2.2_N)/(r2.1_N + r2.2_N), # acute 2
                                                                (4*r1.1_N*r2.2_N)/(r1.1_N + r2.2_N)))) # acclimation
    
    lnVRQ10_data <- lnVRQ10_data %>%  
              mutate(n_ef = ifelse(name == "lnVR_Q10_acute_t1", (4*r1.1_N*r1.2_N)/(r1.1_N + r1.2_N), # acute 1
                            ifelse(name == "lnVR_Q10_acute_t2", (4*r2.1_N*r2.2_N)/(r2.1_N + r2.2_N), # acute 2
                                                                (4*r1.1_N*r2.2_N)/(r1.1_N + r2.2_N)))) # acclimation
  
  model_full_lnrr_pb <- brms::brm(effect_size | se(sqrt(V)) ~  max_temp_c + c_accl_time + sqrt(1/n_ef) + habitat*life.hist*type + trait_category + habitat:type + habitat:life.hist + habitat:life.hist:type + (~1 | record_num) + (1 | gr(species_full, cov = A)) + (1 + max_temp_c | spp) + (1 | trait) + (1  | obs),  data2 = list(A=A), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 8, data = lnRRQ10_data)

  #Pull the posterior for the slope with sqrt effective n
  post_pb_lnrr <- posterior_samples(model_full_lnrr_pb, "^b_sqrt1Dn_ef")[,1]
  
  model_full_lncvr_pb <- brms::brm(effect_size | se(sqrt(V)) ~ max_temp_c +  c_accl_time + sqrt(1/n_ef) + habitat*life.hist*type + trait_category + (~1 | record_num) + (1 | gr(species_full, cov = A_CVR)) + (1 + max_temp_c| spp) + (1 | trait) + (1  | obs),  data2 = list(A_CVR=A_CVR), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 8, data = lnCVRQ10_data)
  
  #Pull the posterior for the slope with sqrt effective n
  post_pb_lncvr <- posterior_samples(model_full_lncvr_pb, "^b_sqrt1Dn_ef")[,1]
  ...
  #Pull the posterior for the slope with sqrt effective n
  model_full_lnvr_pb <- posterior_samples(model_full_lnvr_pb, "^b_sqrt1Dn_ef")[,1]
...
Funnel plots did not show any noticeable deviation from the typical funnel shape for any of the effect size estimates (@fig-s2). Meta-regression models including sampling standard error as a moderator also suggested no relationship with effect size for $lnRR_{Q_{10}}$ ($\beta$ = `r mean(post_pb_lnrr)`, 95% CI: `r quantile(post_pb_lnrr, 0.025)` to `r quantile(post_pb_lnrr, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(post_pb_lnrr))`), $lnCVR_{Q_{10}}$ ($\beta$ = `r mean(post_pb_lncvr)`, 95% CI: `r quantile(post_pb_lncvr, 0.025)` to `r quantile(post_pb_lncvr, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(post_pb_lncvr))`) or $lnVR_{Q_{10}}$ ($\beta$ = `r mean(model_full_lnvr_pb)`, 95% CI: `r quantile(model_full_lnvr_pb, 0.025)` to `r quantile(model_full_lnvr_pb, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(model_full_lnvr_pb))`) was not significant indicating little evidence for publication bias.
