...
## Terrestrial Ectotherms
  terrest_data <- sub_data  %>% filter(habitat == "t")
    new_data_t <- data.frame(
      spp = terrest_data$spp,
      trait = terrest_data$trait,
      habitat = terrest_data$habitat,
      c_accl_time = 0,
      V = 0,
      lat = terrest_data$lat.y,
      long = terrest_data$long.x
    )

terre_pred <- predict(model5_bayes, newdata = new_data_t, summary = FALSE)
