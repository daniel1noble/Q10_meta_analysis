...
Seebacher^3^ $\dagger$, & Shinichi Nakagawa^2,5^ $\dagger$
...
To better understand the characteristics of performance curves among individuals in a sample that would lead to observed changes in variance across temperature we conducted a simple simulation. To simulate performance curves, we used an asymmetrical Gaussian function (@angilletta2009thermal):

$$
P_{T} = 2\epsilon^{-\frac{(T-\delta)^2}{2\sigma^2}}\Phi\left(\alpha\frac{T-\delta}{\sigma}\right)
$$ {#eq-sim}

where $T$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where performance is maximized), $\sigma$ the performance breadth, and $\alpha$ the skewness of the performance function or rate variation (see @fig-sim in *Supplementary Materials* for example curves).  We simulated *n* = 1000 individual performance curves by varying the amount of between individual variance on each of the key parameters ($\delta$, $\sigma$, $\alpha$) in all possible combinations from 0.01 to 2. We then took the standard deviation of the performance at two temperatures 10°C apart (18 and 28°C) to calculate $lnVR_{Q_{10}}$ and identify parameter space that matches observed patterns in our empirical data. 
...
#| label: fig-phylo
#| echo: false
#| fig-cap: "Phylogenetic distribution of acute and acclimation $lnRR_{Q_{10}}$/$lnVR_{Q_{10}}$ estimates across major habitats.  The total number of acute and acclimation  $lnRR_{Q_{10}}$/$lnVR_{Q_{10}}$ effect sizes are indicated by the coloured bars as well as whether the taxa are marine, freshwater or terrestrial. Silhouettes are only representative taxa of major clades within the tree."
...
We show that biochemical traits (e.g., enzymes) were more likely to acclimate compared to whole-organisms traits as predicted, but only for marine ectotherms. In contrast, both whole-organism and biochemical traits were equally likely to acclimate in freshwater ectotherms. Unsurprisingly, we observed no distinction between traits in terrestrial ectotherms......FRANK?
...
In contrast to acclimation capacity, variance in physiological rates increased across habitats with effect sizes being 3-5 times larger than those observed for acclimation. Mechanistically, it is unclear what is contributing to the increased variation in physiological rates at higher temperatures, but it could be the result of increased among-individual variability in how biochemical, cellular and physiological processes function at higher temperatures to maintain homeostasis [@tattersall2012coping; @angilletta2009thermal; @somero1995proteins; @fields2001protein]. Higher temperatures increase membrane fluidity affecting  gradients and impacting protein structure and function [@somero1995proteins; @fields2001protein; @tattersall2012coping]. Such challenges (among others) may expose among-individual variation within a population. Indeed, even acclimation itself can impact among-individual variation [REF FRANK?], and we find evidence to that effect given that biochemical processes in marine organisms were both more likely to acclimate and have reduced variance increases compared to acute responses. However, this was not a general phenomena given that freshwater ectotherms were able to maintain high variance despite strong acclimation responses. 

Importantly, increased variance in physiological rates was not equal among ectotherms occupying terrestrial, marine and freshwater habitats, with increases in variance being higher in freshwater ectotherms (~60% increase / 10°C) compared to terrestrial ectotherms (~36% increase / 10°C). One possible hypothesis for the differences in variability we observed across habitats could be that among-individual variation in key parameters affecting the shape of thermal performance curves differ [@angilletta2009thermal; @huey1989evolution]. Our simulations suggest that theoretical and observed $lnVR_{Q_{10}}$ match when thermal performance curves have different among-individual variance in thermal optima and breadth making this hypothesis plausible. Such patterns across habitats are expected given that terrestrial ectotherms should be adapted warmer and more variable environments. Theoretical models also suggest that populations with greater temporal environmental variation exhibit greater thermal breadth [@lynch1987environmental]. While we did not find support that thermal variation co-varied with $lnVR_{Q_{10}}$ (see below), as would be expected, such analyses will depend on an appropriate measure of temporal variation in temperature that is biologically relevant - a challenging feat across diverse taxa but worthy of future investigation. 

Our results further highlight the potential vulnerability of terrestrial ectotherms to climate change. Assuming changes in variation in physiological rates are underpinned by genetic variation, and there is a genetic correlation with fitness, such reductions in physiological variance could limit adaptation in terrestrial habitats more than aquatic habitats in the future [@urban2023and]. For example, under climate change we expect an increase in variance in physiological rates of only ~13% in terrestrial habitats whereas for freshwater habitats we expect variation in physiological rates to increase by ~30%. Importantly, there will be limits to variance increases, and we predict that organisms closer to their upper thermal limits ($CT_{max}$) will have lower $lnVR_{Q_{10}}$ values compared to those farther away from $CT_{max}$. Some evidence points to possible differences across habitats in upper thermal limits already [@Gunderson2015-hz; @Pinsky2019-fn] making this a fruitful future question to ask.
...
################## 
  ...
  ggsave(p3b, filename = "./output/figures/figS6.pdf", width = 11.5, height = 8)
