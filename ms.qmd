...
# Now that we have all the predictions we can merge the data together. Note that species itself is not really that useful because each prediction at each latitude and longitude should incoperate the species effect. So we'll remove it. While I retain spp, it's still not totally specific to the species, but left it there so it's clear how these data match with the wild data. Note there are only 134 species because this is subset of the wild data.
  all_pred <- rbind(terrest_data, marine_data, fresh_data) %>% dplyr::select(-species_full)
  spp_wild  <- length(unique(sub_data$spp))
```

Acute and acclimation responses for wild ectotherms were much less common than studies done on captive populations ($N_{species}$ = `r length(unique(pop_spp_w$Species))`, from `r dim(pop_spp_w)[1]` populations). Globally, there was a clear bias towards spiecies in the Northern Hemiphere (@fig-preds). Predictions of current global changes in the opportunity for selection were generally conservative with our model explaining ~ 50% of the variation in the observed data ($R^2$ = `r round(mean(r2_2), 2)`, 95% CI: `r round(quantile(r2_2, 0.025)[1], 2)` to `r round(quantile(r2_2, 0.975)[1], 2)`). 

Projected changes in the opportunity for selection were highly variable across the globe, with some regions showing a decrease in the opportunity for selection, while others showed an increase (@fig-future). Overall, there was a net decrease in the opportunity for selection across the globe ()
...
map1_current <- ggplot() +
  ...
  geom_point(
    ...
    alpha = 0.7, size = 5
  ) + labs(x = "Longitude", y = "Latitude", color = TeX("$lnCVR_{Q_{10}}$")) + scale_color_viridis() + theme_map(base_size = 22)
...
p <- (zoom_plots[[1]] | zoom_plots[[2]]) / map1_current  + plot_layout(heights  = c(1, 2)) + plot_annotation(tag_levels = "A", tag_suffix = ")") & theme(plot.tag = element_text(size = 20, face = "bold"), plot.margin = unit(c(0, 0, 0, 0), "cm"))

ggsave(p, filename = "./output/figures/fig-preds.png", width = 9.1, height = 7.2)
...
# Clean up and create a match column. We need to get the last 5 years current to average temp and then convert this to Celcius
  climate2_future <- climate %>% mutate(match = interaction(Species, Latitude, Longitude)) %>% dplyr::select(match, Species, Latitude, Longitude, grep("X2018|X2019|X2020|X2021|X2022", colnames(climate)))  %>% mutate(mean = apply(.[, grep("X2018|X2019|X2020|X2021|X2022", colnames(.))], 1, function(x) mean(x, na.rm = TRUE)), Celc_curr = mean - 273.15) %>% dplyr::select(-grep("X2018|X2019|X2020|X2021|X2022", colnames(.)))

# Now we can merge teh climate datasets together so that we have the species, lat and long, current and future climate. Then we can calculate the expected 'increase' in T into the future
...
# Now lets merge all_pred with the climate projections
  # Simplify columns 
  climate_data_new <- climate_data  %>% dplyr::select(-Latitude, -Longitude, -Species)
...
# Need to take a step back
  # 1. Why do we have NA in this data set? 
  # 2. Why are l95 and u95 for marine not working
  # 3. Apply to the posteriors, rather than summary.
