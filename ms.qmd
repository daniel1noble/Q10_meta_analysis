...
#| label: fig-fig8
#| fig-align: center
#| fig-cap: Bubble plot environment lnRR for wild populations by habitat
...
dat_text <- data.frame(label = c(text(slope_f), text(slope_m), text(slope_t)),
 ...
 col <- c("#0871B9", "#F3B40D", "#1BB908")
p8.1 <- bubble_plot(model3.9, mod = "cv_c", group = "record_num", by = "habitat", data = lnRRQ10_data_wild_acclim, ylab = TeX("$lnRR_{Q_{10}}$"), xlab = "Thermal Coefficient of Variation (CV)", condition.nrow = 1) + 
theme_classic() + 
scale_fill_manual(values = c(col)) + 
geom_text(data = dat_text, aes(x = x, y = y, label = label), size = 7) + 
geom_text(data = dat_text, aes(x = x, y = y-0.2, label = label2), size = 7) +
geom_text(data = dat_text, aes(x = x, y = y-0.4, label = p), size = 7) +
geom_text(data = dat_text, aes(x = x, y = y+4, label = title), size = 7) + 
...
theme_classic() + 
scale_fill_manual(values = c(col)) + 
geom_text(data = dat_text2, aes(x = x, y = y, label = label), size = 7) + 
geom_text(data = dat_text2, aes(x = x, y = y-0.2, label = label2), size = 7) +
geom_text(data = dat_text2, aes(x = x, y = y-0.4, label = p), size = 7) +
...
p8.1.2 <- (p8.1 / p8.2) + plot_annotation(tag_levels = "A", tag_suffix = ")")

ggsave(p8.1.2, width = 13, height = 12.5, filename = "./output/figures/fig8.png")
...
# We'll take the marginalised mean for each type as there is no difference between acclimation and acute really except for in freshwater, but this effect size is so tiny. It's a 10% reduction at high temps and acclim is only a 2% reduction in variance at high temps.
  ...
  # Lets now convert these to a %
    f_VR_mean_ef  <- abs((exp(f_VR_mean) - 1)*100) # ~8.4% reduction
    m_VR_mean_ef  <- abs((exp(m_VR_mean) - 1)*100) # ~15% reduction
    t_VR_mean_ef  <- abs((exp(t_VR_mean) - 1)*100) # ~30% reduction
...
################## 
  ...
  annotate("text", x = 1.15, y = -3.7, label = paste0("P = ", round(pmcmc(f_VR_mean), digit = 2)), hjust = 0, size = 6) + 
  annotate("text", x = 1.15, y = -5, 
  ...
  annotate("text", x = 2.15, y = -3.7, label = paste0("P = ", round(pmcmc(m_VR_mean), digit = 2)), hjust = 0, size = 6) + 
  annotate("text", x = 2.15, y = -5, 
  ...
  annotate("text", x = 3.15, y = -5, 
    ...
    annotate("text", x = 3.15, y = -3.7, label = paste0("P = ", round(pmcmc(t_VR_mean), digit = 3)), hjust = 0, size = 6) + scale_fill_manual(values = c(col))
