...
if(rerun){
  ...
  model1.3_m_het <- metafor::rma.mv(effect_size ~  max_temp_c + c_accl_time + type + trait_category + type:trait_category, V = V, 
                              ...
                              struct = "HCS", rho = 0, phi = 0, dfs = "contain", 
...
$lnRR_{Q_{10}}$, $lnCVR_{Q_{10}}$ and $lnVR_{Q_{10}}$, as with $Q_{10}$ more generally, all assume that the effect of temperature on physiological rates (or changes in relative variance) is log-linear. While $Q_{10}$ [@van1884etudes] has been used exstensively in the physiological literature to successfully address a multitude of questions [e.g., @Havird2020-mj; @Seebacher2015-wi], there is a preference for using a Boltzmann – Arrhenius (BA) relationship (or its more recent extension, the Sharpe-Schoolfield model [@molnar2017thermal; @michaletz2024hotter]) to model temperature effects on physiological rates [@gillooly2001effects; @michaletz2024hotter]. Over biologically realistic temperature ranges the BA and $Q_{10}$ approaches can vary at times by as much as 15% [@gillooly2001effects]. However, debate still exists over the utility of $Q_{10}$ when modelling temperature-dependence and both BA and $Q_{10}$ can exhibit curvilinearity at normal operating temperatures (as discussed in [@michaletz2024hotter]). @white2012information also showed that the BA model may not always perform better. For example, in eukaryotes the $Q_{10}$ approach provided a 5.8 fold better fit to metabolic rate data than the BA relationship [@white2012information]. 

Given that we restricted studies only to those that manipulated temperatures within normal operating ranges for a species and we only focus on eukaryotes, $Q_{10}$ should remain an appropriate way to capture temperature-dependent effects [@michaletz2024hotter]. However, given that it is not possible to collect full thermal performance curves for species within our data we could not validate this assumption directly. However, to correct mean effect size in our analysis as temperatures increase we included the maximum temperature of the experimental manipulations in our models. If the log-linearity assumption is violated then $lnRR_{Q_{10}}$ should decrease for studies applying higher temperature treatments because such temperatures are expected to either converge on or cross the thermal optima of the performance curve causing reaction rates to decelerate or decrease beyond $T_{op}$ [@michaletz2024hotter]. The benefit of such an approach means that we could still take advantage of the excellent properties of $Q_{10}$ as an effect size while statistically correcting for the potential non-linearity that would be expected in the data at high temperatures. Given that our data included a wide range of species and habitats, we also included a random slope of maximum temperature that varied across species because we, *a priori*, expected that each species will vary in their thermal performance curve and the chosen temperatures experimentally manipulated along the performance curve. To improve model interpretation we centred the maximum temperature around the mean temperature of the experimental treatments.
