...
To better understand the characteristics of the performance curves in a sample that would lead to observed changes in variance (and relative variance) across temperature we conducted a simple simulation. To simulate the performance curves, we used a asymmetrical Gaussian function (@eq-sim):

$$
P_{z} = 2\epsilon^{-\frac{(z-\delta)^2}{2\sigma^2}}\Phi\left(\alpha\frac{z-\delta}{\sigma}\right)
$$ {#eq-sim}

where $z$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where performance is maximised), $\sigma$ is the performance breadth, and $\alpha$ is the skewness of the performance function or rate variation.  

We first explored four hypothetical examples to identify key parameters (@fig-sim). We simulated 40 individuals with varying performance breadth, optima and rate variation. We then calculated the relative variance in performance across the temperature gradient as the variance in performance at each temperature divided by the maximum performance at that temperature.

We then conducted a larger simulaton manipulating the level of between individual variation in the optima and performance breadth while holding constant rate variation because optima and breadth appeared to be most important (@fig-sim). We simulated *n* = 1000 individual performance curves and took the mean and standard deviation of the performance at two temperatures 10 degrees apart (18 and 28 degrees) to calculate the log coefficient of variation (CV) and log variance ratio in performance.

Our simulations recovered parameter space for $lnVR_{Q_{10}}$ that matched our empirical results (@fig-sim2). 
...
ggsave(p_sim, width = 18.5, height = 18, filename = "./output/figures/figS12.png")
...
#| label: fig-sim2
#| fig-align: center
#| fig-cap: "Expected $lnCVR_{Q_{10}}$ and $lnVR_{Q_{10}}$ when varying $\\sigma_{_b_{\\delta}}$ = {0.01, 2} and  $\\sigma_{_b_{\\sigma}}$ = {0.01, 2} while fixing $\\sigma_{_b_{\\alpha}}$ = 0.05. In all simulations, $\\delta$ = 35, $\\sigma$ = 9, $\\alpha$ = -15, and *n* = 1000 individuals were simulated for each combination of $\\sigma_{_b_{\\delta}}$, $\\sigma_{_b_{\\sigma}}$ and $\\sigma_{_b_{\\alpha}}$."
...
# Simulation 1. Assume optimum is rather constant among individuals 
#parameters
delta =  35 + rnorm(n,0,5) 
sigma =  20 +rnorm(n,0,0.1)
alpha = 0.0001 +rnorm(n,0,1)

ws1<- list()
for(i in 1:length(delta)){
ws1[[i]] <- 2*exp(-(z-delta[i])^2/(2*sigma[i]^2))*pnorm(alpha[i]*(z-delta[i])/sigma[i])
}
...
ggplot(ws1, aes(x = z, y = fitness, color = run)) + geom_line() + theme_classic() + labs(y = "Physiological Rate", x = TeX("Temperature ($^\\circ$C)"), color = "Individual", title = "Thermal Breadth") + scale_color_viridis_d() + my_theme()+ geom_vline(xintercept = 28, linetype = "dashed", color = "red") +     geom_vline(xintercept = 18, linetype = "dashed", color = "blue")   + my_annotate(sum_sim4, 10)
...
# Sharpe-Schoolfield Arrhenius equation
...
per <- sharpeschoolfull_1981(temp, r_tref, e, el, tl, eh, th, tref)

# Create a data frame for plotting
df <- data.frame(Temperature = temp, Rate = per)


ggplot(df, aes(x = Temperature, y = Rate)) +
  geom_line(color = "blue") +
  theme_minimal() +
  labs(title = "Sharpe-Schoolfield Arrhenius Equation",
       x = "Temperature (K)",
       y = "Rate")
...
#C, constant in shifts of vertical axis
#Tth, temperature of thermal threshold
#d, constant in shifts of horizontal axis

pf <- function(T, Tthres, d, C, QT){
  therm <- C*exp((T*log10(QT))/10)
      d <- ifelse(T<Tthres, 1, (1-d*(Tthres - T)^2))
 return(therm*d)
}
...
pf1 <- c()
for(i in 1:length(T)){
  pf1[i] <- pf(T[i], Tthres, d, C, QT)
}
plot(T, pf1, type = "l", xlab = "Temperature", ylab = "Performance", main = "Performance Curve")
