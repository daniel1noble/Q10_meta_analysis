...
We explored the possibility for publication bias graphically using funnel plots, and more formally by including sampling variance (or sampling standard error) in our meta-regression models  [@Nakagawa2022-gy]. Funnel plot asymmetry may suggest a form of publication bias called the 'file-drawer' effect whereby low-powered studies are less likely to be published. To test whether sampling variance covaried with effect size we included it in a multi-level meta-regression model that accounted for all the random effects (study, species, trait) and fixed effects (acclimation time, type of effect, habitat, trait category and the interaction between habitat type and trait category). 
...
# Terrestrial
  p_t_rr <-  bubble_plot(model3.9_t, mod = "cv_c", group = "spp", ylab = TeX("$lnRR_{Q_{10_{acclim}}}$"), xlab = "Thermal Coefficient of Variation (CV)", g = TRUE) + 
    geom_point(data = lnRRQ10_data_wild_acclim_t, aes(x = cv_c, y = effect_size, size = 1/sqrt(V)), colour = col[3]) + 
  scale_fill_manual(values = "black")+ labs(title = dat_text$title[3]) +
...
p_m_rr <-  bubble_plot(model3.9_m, mod = "cv_c", group = "spp", ylab = TeX("$lnRR_{Q_{10_{acclim}}}"), xlab = "Thermal Coefficient of Variation (CV)", g = TRUE) + 
    geom_point(data = lnRRQ10_data_wild_acclim_m, aes(x = cv_c, y = effect_size, size = 1/sqrt(V)), colour = col[2]) + 
  scale_fill_manual(values = "black") + labs(title = dat_text$title[2]) +
...
p_f_rr <-  bubble_plot(model3.9_f, mod = "cv_c", group = "spp", ylab = TeX("$lnRR_{Q_{10_{acclim}}}"), xlab = "Thermal Coefficient of Variation (CV)", g = TRUE) + 
    geom_point(data = lnRRQ10_data_wild_acclim_f, aes(x = cv_c, y = effect_size, size = 1/sqrt(V)), colour = col[1]) + 
  scale_fill_manual(values = "black") + labs(title = dat_text$title[1]) +
...
# Freshwater
  ...
  p_f_cv <-  bubble_plot(model4.9_f, mod = "cv_c", group = "spp", ylab = TeX("$lnCVR_{Q_{10_{acclim}}}$"), xlab = "Thermal Coefficient of Variation (CV)", g =FALSE, k = FALSE) + 
      geom_point(data = lnCVRQ10_data_wild_acclim_f, aes(x = cv_c, y = effect_size, size = 1/sqrt(V)), colour = col[1]) + 
...
geom_point(aes(size = 1/sqrt(V), colour = habitat), alpha = 0.8) +
...
ggplot2::geom_smooth(data = mod_table, ggplot2::aes(x = moderator, y = estimate), method = "loess", 
            ...
            scale_colour_manual(labels = c("f" = "Freshwater", "m" = "Marine", "t" = "Terrestrial"), values = col)+ 
...
#| fig-cap: "Bubble plot of the relationship between $lnRR_{Q_{10}}$ and acclimation time for terrestrial (green), marine (orange) and freshwater (blue) habitats. Acclimation time is centered around the mean acclimation time (37.5 days) in the data. Not all studies reported acclimation time hence the total number of effects, k, was 1767."
  ...
  bubble_plot(model1.2, mod = "c_accl_time", group = "spp", ylab = TeX("$lnRR_{Q_{10}}$"), xlab = "Centered Acclimaton Time (days)") + geom_point(data = lnRRQ10_data, aes(x = c_accl_time, y = effect_size, colour = habitat, fill = habitat)) + scale_colour_manual(labels = c("f" = "Freshwater", "m" = "Marine", "t" = "Terrestrial"), values = col) + theme_classic() + theme(axis.title  = element_text(size = 30), 
                        axis.text = element_text(size = 26), 
                        axis.text.y = element_text(size = 26),
                        legend.text = element_text(size = 24), 
                        legend.title = element_text(size = 26))
...
Theoretical models highlight the importance of environmental predictability in selecting for plastic responses. However, capturing environmental predictability is challing given that it is unclear which timescale one should select. For example, is it more important to look at correlation between temperatures monthly or seasonally. In addition, such temporal resolution will likely depend on the species in question given that for some species fine-grained thermal predictability maybe more important compared to others.
...
if(rerun){
  # More realistic model with 
  model3.8  <- metafor::rma.mv(effect_size ~ max_temp_c + c_accl_time + acf_all_c + habitat + habitat:acf_all_c, V = V,
                                       random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | trait,
                                            ~1 + max_temp_c| spp,
...
if(rerun){
    # Terrestrial Models
    model3.8_t  <- metafor::rma.mv(effect_size ~ max_temp_c + c_accl_time + acf_all_c, V = V,
                                       random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | trait,
                                            ~1 + max_temp_c| spp,
    ...
    # Freshwater Models
    model3.8_f  <- metafor::rma.mv(effect_size ~ max_temp_c + c_accl_time + acf_all_c, V = V,
                                       random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | trait,
                                            ~1 + max_temp_c| spp,
    ...
    # Marine Models
      model3.8_m  <- metafor::rma.mv(effect_size ~ max_temp_c + c_accl_time + acf_all_c, V = V,
                                       random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | trait,
                                            ~1 + max_temp_c| spp,
...
#| label: fig-4
#| fig-align: center
#| fig-cap: "Predicted mean (thick black line) $lnRR_{Q_{10}}$ as a function of the Thermal Coefficient of Variation (CV) (A) and thermal predictability (B) for wild populations across marine, freshwater and terrestrial habitats. Dashed lines indicate 95% confidence intervals and dotted lines indicate 95% prediction intervals. Model slope ($\\beta$) along with the 95% CI and $p_{MCMC}$ value is shown for each habitat."
...
# lnRR ACF
 ...
 dat_text2rr <- data.frame(label = c(text(slope_f_3.8), text(slope_m_3.8), text(slope_t_3.8)),
...
geom_text(data = dat_text2rr, aes(x = x+1, y = y, label =label), size = 5) + 
geom_text(data = dat_text2rr, aes(x = x+1, y = y-0.3, label =label2), size = 5) +
geom_text(data = dat_text2rr, aes(x = x+1, y = y-0.65, label = p), size = 5) +
...
# lnCVR ACF
dat_text2cvr <- data.frame(label = c(text(slope_f_4.8), text(slope_m_4.8), text(slope_t_4.8)),
...
theme_classic() + 
scale_fill_manual(values = c(col)) + 
geom_text(data = dat_text2cvr, aes(x = x+1, y = y, label = label), size = 5) + 
geom_text(data = dat_text2cvr, aes(x = x+1, y = y-0.6, label = label2), size = 5) +
geom_text(data = dat_text2cvr, aes(x = x+1, y = y-1.2, label = p), size = 5) +
theme(legend.position = "bottom", 
      strip.background = element_blank(), 
      strip.text = element_blank(), 
      axis.text = element_text(size = size_axis), 
...
p8.1.2 <- (p8.1 / p8.2) + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 20))
