...
# Create the table
   ...
   tabs1$Parameter <- c("Intercept", "Acclimation Time (z scaled)", "Acclimation Effect", "Habitat (Marine)", "Habitat (Terrestrial)", "Acclimation*Marine", "Acclimation*Terrestrial", "Study", "Phylogeny", "Species", "Trait")

    flextable(tabs1) %>% flextable::bold(part = "header") %>% flextable::autofit() %>% flextable::add_body_row(values = as_paragraph(as_b("Fixed Effects")), top = TRUE, colwidths = 5) %>% flextable::compose(value = as_paragraph(as_b(c(("Random Effects"), rep("", 4)))), i = 8)
```
