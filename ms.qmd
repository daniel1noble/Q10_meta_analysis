...
Climate change is expected to result in warmer and more variable thermal environments globally. Greater thermal variability is expeced to result in strong selection pressures leading to genetic adaptation and/or the evolution of adaptive phenotypic plasticity. However, such responses depend on the extent of genetic and phenotypic variability, yet most work has focused on mean responses to climate warming ignoring how temperature changes variability. Here, we collect data on marine, freshwater and terrestrial ectotherms to quantify their capacity for acclimation and the extent to which variance in physiological rates change in response to higher temperatures. We use new effect size measures and meta-analysis to understand 
...
# % change in mean acclimation relative to acute
  change_f <- (exp(f_acclim - f_acute)-1)*100; quantile(change_f, c(0.025, 0.975)) 
  change_m <- (exp(m_acclim - m_acute)-1)*100; quantile(change_m, c(0.025, 0.975))
  change_t <- (exp(t_acclim - t_acute)-1)*100; quantile(change_t, c(0.025, 0.975))
  ...
  (f_acclim - f_acute) / mean(c(f_acclim, f_acute))
# % change on Q10 scale
...
Overall, $lnRR_{Q_{10_{acclimation}}}$ and $lnRR_{Q_{10_{acute}}}$ differed by only `r mean(abs(avg_change))`% across all habitats (95%CI: `r quantile(abs(avg_change), c(0.025, 0.975))[1]` to `r quantile(abs(avg_change), c(0.025, 0.975))[2]`%). Ectotherms in marine and freshwater environments showed partial compensation of physiological rates ([@fig-1]B) amounting to reduced $lnRR_{Q_{10_{acclimation}}}$ of `r mean(abs(change_f))`% (95% CI: `r quantile(abs(change_f), c(0.025, 0.975))[1]` to `r quantile(abs(change_f), c(0.025, 0.975))[2]`) in freshwater and `r mean(abs(change_m))`% (95% CI: `r quantile(mean(abs(change_m), c(0.025, 0.975))[1]` to `r quantile(mean(abs(change_m), c(0.025, 0.975))[2]`) in marine environments. In contrast, terrestrial ectotherms showed no acclimation (possibly even inverse acclimation) -- showing a `r mean(abs(change_t))`% increase in $lnRR_{Q_{10_{acclimation}}}$ (95% CI: `r quantile(abs(change_t), c(0.025, 0.975))[1]` to `r quantile(abs(change_t), c(0.025, 0.975))[2]`, [@fig-1]B). Considering acute responses of animals acclimated to high temperatures are generally slightly elevated compared to cold acclimated animals (~7%; $\beta$ = `r model1.s1$b[3]`, 95% CI: `r model1.s1$ci.lb[3]` to `r model1.s1$ci.ub[3]`, p `r p_value(model1.s1$pval[3])`), acclimatization is not likely going to provide adaptive benefits under climate change.
...
annotate("text", x = 3.01, y = 4.0, 
         label =paste0(round(mean(change_t), 2), "%"), hjust = 0, size = 6) +
  annotate("text", x = 2.85, y = 3, label = "P = 0.36", hjust = 0, size = 6) + scale_fill_manual(values = c(col)) + ylim(-0.8, 5.05) +  annotate("text", label= paste0("k = ", k_lnrr$k, " (", k_lnrr$sdy, ")"), x = k_lnrr$moderator, y = 5)
    
  p1.2 <- p1.2 + theme_classic() + theme(axis.title  = element_text(size = 24),
        axis.text = element_text(size = 15))

  ggsave(p1.2, filename = "./output/figures/fig1.png", width = 12, height = 9.6)
...
################## 
  ...
  p2 <- orchard_plot(model2.2, xlab = TeX("$lnCVR_{Q_{10}}$"), mod = "habitat", group  = "record_num", weights = "prop", angle = 45, trunk.size = 8, branch.size = 2) +
              scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) + annotate("text", x = 1.3, y = -1.8, 
  ...
  annotate("text", x = 1.15, y = -1.5, label = paste0("P = ", round(pmcmc(f_cvr_mean), digit = 2)), hjust = 0, size = 6) + 
  annotate("text", x = 1.15, y = 1, 
  ...
  annotate("text", x = 2.15, y = -1.5, label = paste0("P = ", round(pmcmc(m_cvr_mean), digit = 2)), hjust = 0, size = 6) +
  annotate("text", x = 2.15, y = 1, 
           label =paste0(round(mean(exp(m_cvr_mean)-1)*100, 2), "%"), hjust = 0, size = 6) + annotate("text", x = 3.4, y = -1.8, 
  ...
  annotate("text", x = 3.25, y = 1, 
    ...
    annotate("text", x = 3.25, y = -1.5, label = paste0("P = ", round(pmcmc(t_cvr_mean), digit = 3)), hjust = 0, size = 6) + scale_fill_manual(values = c(col)) + ylim(-2, 2) + 
    annotate("text", label= paste0("k = ", k_cvr$k, " (", k_cvr$sdy, ")"), x = c(1.15, 2.15, 3.25), y = 1.8)
  ...
  p2 <- p2 + theme_classic() + theme(axis.title  = element_text(size = 24),
     ...
     ggsave(p2, filename = "./output/figures/fig2.png", width = 10, height = 9.5)
...
ptm <- orchard_plot(model1.3_m_het, mod = "trait_category", at = list(type = c("acute", "acclim")), by = "type", group = "record_num", weights = "prop",  xlab = TeX("$lnRR_{Q_{10}}$"), angle = 0, condition.lab = "Type") +
  scale_x_discrete(labels = c("Mito_leak" = "Proton Leak",
                              ...
                              "Mito_oxidation" = "OXPHOS")) + theme_classic() + theme(legend.position = "none",
                              axis.text = element_text(size=12)) + ylim(-1.5, 2.5) +
                            annotate("text", label= paste0(k_marine$k, " (", k_marine$sdy, ")"), x = k_marine$moderator, y = 2.4)
