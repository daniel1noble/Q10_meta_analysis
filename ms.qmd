...
##################################
    pacman::p_load(tidyverse,  metafor, flextable, raster, rasterVis,  patchwork, RColorBrewer, ggtree, ape, phytools, viridis, MCMCglmm, brms, latex2exp, orchaRd, multcomp, ggbeeswarm, magick, ggimage, gt, performance)
...
######################################################################
# Model predcitions for terrestrial, marine and freshwater ectotherms
######################################################################
...
test <- sub_data  %>% filter(habitat == "t") %>% distinct(spp, lat.y, long.x)
length(unique(test$spp))

## Terrestrial Ectotherms
  terrest_data <- sub_data  %>% filter(habitat == "t")  %>% distinct(spp, habitat, lat.y, long.x)
  ...
  terre_pred <- predict(model5_bayes, newdata = new_data_t2, summary = TRUE, allow_new_levels = TRUE, robust = FALSE)

  # Test manual calculations. We can also set summary = FALSE and then do the calculations ourselves. Note we want robust = FALSE to make the comparisons more similar as otherwise the calculation are different when using summary TRUE/FALSE  
  terre_pred2 <- predict(model5_bayes, newdata = new_data_t2, summary = FALSE, allow_new_levels = TRUE, robust = FALSE)
  plyr::ldply(apply(terre_pred2, 2, function(x) post_summary(x)))


## Marine ectotherms  
  marine_data <- sub_data  %>% filter(habitat == "m")
    new_data_m <- data.frame(
      spp = marine_data$spp,
      trait = marine_data$trait,
      habitat = marine_data$habitat,
      c_accl_time = 0,
      V = 0,
      lat = marine_data$lat.y,
      long = marine_data$long.x
    )

marine_pred <- predict(model5_bayes, newdata = new_data_m, summary = FALSE)

## Freshwater ectotherms
  fresh_data <- sub_data  %>% filter(habitat == "f")
    new_data_f <- data.frame(
      spp = fresh_data$spp,
      trait = fresh_data$trait,
      habitat = fresh_data$habitat,
      c_accl_time = 0,
      V = 0,
      lat = fresh_data$lat.y,
      long = fresh_data$long.x
    )

fresh_pred <- predict(model5_bayes, newdata = new_data_f, summary = FALSE)
