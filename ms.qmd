...
##################################
    pacman::p_load(tidyverse,  metafor, flextable, raster, rasterVis,  patchwork, RColorBrewer, ggtree, ape, phytools, viridis, MCMCglmm, brms, latex2exp, orchaRd, multcomp, ggbeeswarm, magick, ggimage, gt, performance, ggthemes)
...
# First, lets get the current and future temperature data for 2081-2085. We'll use the mean. 
  future_temp <- read.csv("./output/climate_data/gfdl2081-2085_air_and_SST_timeseries.csv")

# Lets clean this data up. We only need the mean temperature here, but this needs to be converted to Celcius as right now it's in Kelvins
  future_temp  <- future_temp  %>% 
                  dplyr::select(Species, Latitude, Longitude, mean)  %>% 
                  mutate(Celc_future = mean - 273.15,
                          match = interaction(Species, Latitude, Longitude))

# Current climate data
  climate <- read.csv("./output/climate_data/ERA5_air_and_SST_timeseries.csv")

# Clean up and create a match column. We need to get the last 5 years current to average temp and then convert this to Celcius
  climate2_future <- climate %>% mutate(match = interaction(Species, Latitude, Longitude)) %>% 
                                dplyr::select(match, Species, Latitude, Longitude, grep("X2018|X2019|X2020|X2021|X2022", colnames(climate)))  %>% 
                                mutate(mean = apply(.[, grep("X2018|X2019|X2020|X2021|X2022", colnames(.))], 1, function(x) mean(x, na.rm = TRUE)), Celc_curr = mean - 273.15) %>% 
                                dplyr::select(-grep("X2018|X2019|X2020|X2021|X2022", colnames(.)))
...
# Now we can merge the climate datasets together so that we have the species, lat and long, current and future climate. Then we can calculate the expected 'increase' in T into the future
  ...
  climate_data <- left_join(climate2_future, future_temp, by = "match")  %>% dplyr::select(match, Latitude.x, Longitude.x, Species.x, Celc_curr, Celc_future)  %>% mutate(T_change = Celc_future - Celc_curr)  %>% rename(Species = Species.x, Latitude = Latitude.x, Longitude = Longitude.x)
...
world_map = map_data("world") %>% 
  ...
  map2_temp_proj <- ggplot() +
    ...
    geom_point(
      ...
      alpha = 0.7, size = 5
    ) + labs(x = "Longitude", y = "Latitude", color = TeX("$\\Delta$ Temp. ($^\\circ$C)"), title = "Current (2018-2022) vs Future (2080-2085)") + scale_colour_gradient2(trans = "reverse") + theme_map(base_size = 22) + theme(plot.title = element_text(hjust = 0.5))

# Now lets merge all_pred with the climate projections
  # Simplify columns 
  climate_data_new <- climate_data  %>% dplyr::select(-Latitude, -Longitude)

  # Mereg with model predcitions. This is the predcited lnCVRq10 for 10C change AT the specific latitude and longitude. We must assume that there is no plasticty or adaptation at each site to make projections
  ...
  # First, lets merge the climate data with the predcitions, but here predcitions are not summarised. We have the entire posteior
    cols_select <- colnames(all_pred2_no_sum) %in% c(1:3200)
   
    all_pred2_no_sum <-  all_pred_no_sum %>% 
                        mutate(match = interaction(spp, Latitude, Longitude))  %>% 
                        left_join(., climate_data_new, by = "match") 
   
  # Second, lets convert each estimate to lncvr 1 degree C insteda of 10
   all_pred2_no_sum2 <- all_pred2_no_sum %>%  
                        mutate_if(cols_select, lncvr_1c) 
  ...
  # Third, lets multiply each estimate by the temperature change at that location. I've checked 4 different cells and they are calculated correctly.
    all_pred2_no_sum2[,cols_select]  <- all_pred2_no_sum2[,cols_select] * all_pred2_no_sum2$T_change

  # Fourth, we can calculate the percentage change on the posterior
    all_pred2_no_sum2[,paste0("perc", c(1:3200))]  <- (exp(all_pred2_no_sum2[,cols_select]) - 1)*100
  ...
  # Now we can summarise each. Here, what we are doing is taking the mean for each column. Effectively pooling across all the iterations for each dataset. This gives us an estimation of the mean change in variance across all locations/species., but it gives us 3200 point estimates of this mean. We can then use this to calculate the 95% CI's.
    mean_perc <-  apply(all_pred2_no_sum2[,paste0("perc", c(1:3200))], 2, mean, na.rm = TRUE)
    mean(mean_perc)
    quantile(mean_perc, 0.025)
    quantile(mean_perc, 0.975)
    hist(mean_perc)

  # Now, lets do this but for each habitat, the good old fashioned way
  mean_perc_habitat <-  lapply(split(all_pred2_no_sum2,  all_pred2_no_sum2$habitat), function(x) apply(x[,paste0("perc", c(1:3200))], 2, mean, na.rm = TRUE))
  lapply(mean_perc_habitat, mean)
  lapply(mean_perc_habitat, function(x) quantile(x, c(0.025, 0.975)))
  #lapply(mean_perc_habitat, hist)
  
  # We can summarise this across the habitat types
    all_pred2_no_sum2  %>% group_by(habitat) %>% summarise(mean = mean(est_per, na.rm = TRUE), 
                                                            l95 = mean(est_perl95, na.rm = TRUE), 
                                                            u95 = mean(est_peru95, na.rm = TRUE))

# Need to take a step back
  # 1. Why do we have NA in this data set? Thats because some species in the Future projections don't have data!
  # 2. Why are l95 and u95 for marine not working...no idea
  # 3. Apply to the posteriors, rather than summary. Working on this....
...
map2_future <- ggplot() +
    ...
    geom_point(
      ...
      alpha = 0.7, size = 5
    ) + labs(x = "Longitude", y = "Latitude", color = TeX("% $\\Delta$"), title = "Future (2080-2085)") + scale_colour_viridis() + theme_map(base_size = 22) + theme(plot.title = element_text(hjust = 0.5))
