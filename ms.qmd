---
title: "Limited physiological plasticity and reduced relative variance in physiological rates of ectotherm populations under climate change"
...
@eq-lnq10VR and @eq-slnq10VR describe the change in physiological rate
variance (@eq-lnq10VR) across a 10°C temperature change along with its
sampling variance (@eq-slnq10VR). While this is a useful metric, as
discussed by @Nakagawa2015-fo there is often a strong mean-variance
relationship that needs to be accounted for in analysing changes in
variance. As such, we calculated the coefficient of variation, which
standardizes changes in variance for changes in means as follows:
...
$lnRR_{Q_{10}}$) or relative variance (i.e., $lnCVR_{Q_{10}}$) is larger
at the higher of the two temperatures (numerator) when standardized to
10°C. Importantly, $lnRR_{Q_{10}}$, $lnCVR_{Q_{10}}$ and
$lnVR_{Q_{10}}$, as with $Q_{10}$ more generally, all assume that the
effect of temperature on physiological rates (or changes in relative
...
### *Relative variation in physiological rates is reduced in terrestrial and marine ectotherms*

Relative variance in physiological rates ($lnCVR_{Q_{10}}$) showed a
decrease with increasing temperature across all habitat types, which was
especially pronounced in terrestrial and marine ectotherms. The total
proportion of heterogeneity in $lnCVR_{Q_{10}}$ was lower compared to
$lnRR_{Q_{10}}$ ($I^2_{sv}$ = `r mean(tot_i2_cvr)`, 95% CI:
`r quantile(tot_i2_cvr, 0.025)` to `r quantile(tot_i2_cvr, 0.975)`),
with most variation being driven by between-study and trait differences
("Model 1"; see Supplement @fig-s3). There were no differences between
$lnCVR_{Q_{10_{acute}}}$ and $lnCVR_{Q_{10_{acclimation}}}$. Reductions
...
Materials*; @tbl-slopes & @fig-mv). There was a `r mean(t_cvr_mean_ef)`%
(95% CI: `r quantile(t_cvr_mean_ef, 0.025)[1]` to
`r quantile(t_cvr_mean_ef, 0.975)[1]`, $p_{MCMC}$ =
`r pmcmc(t_cvr_mean)`) reduction in relative physiological rate variance
for terrestrial ectotherms and a `r mean(m_cvr_mean_ef)`% (95% CI:
`r quantile(m_cvr_mean_ef, 0.025)[1]` to
`r quantile(m_cvr_mean_ef, 0.975)[1]`, $p_{MCMC}$ =
`r pmcmc(m_cvr_mean)`) reduction in relative variation for marine
...
(`r mean(f_cvr_mean_ef)`%, 95% CI: `r quantile(f_cvr_mean_ef, 0.025)[1]`
to `r quantile(f_cvr_mean_ef, 0.975)[1]`, $p_{MCMC}$ =
`r pmcmc(f_cvr_mean)`) (results from "Model 2").

Across habitats whole-organism versus biochemical processes showed a
similar decrease in relative variance in physiological rates
...
# Full fig
############
figbtrait <-(p3 | p1 | p2) / (p3_cvr | p1_cvr | p2_cvr) + plot_annotation(tag_levels = "A") 
...
fig4 <-(p3 | p1 | p2) / (p3_cvr | p1_cvr | p2_cvr) + plot_annotation(tag_levels = "A") 
...
#### *Acute and acclimation* $lnRR_{Q_{10}}$ for detailed trait categories across marine, freshwater and terrestrial taxa
...
Analysis of $lnVR_{Q_{10}}$ suggested that variance increases with
higher temperatures across all habitat types, with terrestrial
ectotherms having the smallest increase in variance (@fig-s1 & @tbl-s1).
...
################## 
  ...
  col <- c("#0871B9", "#F3B40D", "#1BB908")
  p3 <- orchard_plot(model3.2, xlab = TeX("$lnVR_{Q_{10}}$"), mod = "habitat", by = "type", group  = "record_num", weights = "prop", angle = 45, trunk.size = 1.8, branch.size = 2.8) +
  ...
  annotate("text", x = 3.20, y = -2, label =paste0(round(mean(t_VR_mean_ef), 1), "%"), hjust = 0, size = 8) + labs(shape = "Type") + scale_fill_manual(values = c(col)) + annotate("text", y = 2.5, x = c(1.3, 2.3, 3.3)+0.1, label = TeX(paste0("\\it{k} = ", k_vr$k, " (", k_vr$sdy, ")")), size = 6)
...
#| label: tbl-s1
#| echo: false
#| tbl-cap: "Model estimates, standard error, and 95% credible intervals comparing changes in acute and acclimation $lnVR_{Q_{10}}$ across habitat types. Model estimates are based off 1,253 effect sizes from 139 studies."
 ...
 # Extract the fixed / population-level effect summaries of the posterior
    ...
    fixed <- cbind(Parameter = c("Intercept", "Mximum Temperature (centered)", "Acclimation Time (centered)", "Acclimation Effect", "Habitat (Marine)", "Habitat (Terrestrial)", "Acclimation*Marine", "Acclimation*Terrestrial"), fixed)
    rownames(fixed) <- NULL

 # Extract the random effect summaries of the posteriors (SDs)
    random <- rbind(header = rep(0, 4), do.call(rbind, apply(posterior_samples(model3.2_bayes, pars = "^sd"), 2,  function(x) post_summary(x))))
    random <- cbind(Parameter = c(" ", "Study", "Phylogeny", "Species (intercept)", "Species (slope max temp)", "Trait"), random)
    rownames(random) <- NULL

# Create the table
    tabs1 <- rbind(fixed, random)

    flextable(tabs1) %>% bold(part = "header") %>% autofit() %>% add_body_row(values = as_paragraph(as_b("Fixed Effects")), top = TRUE, colwidths = 5) %>% compose(value = as_paragraph(as_b(c(("Random Effects"), rep("", 4)))), i = 10)
```

```{r, fig-lnvrlh}
#| label: fig-lnvrlh
#| echo: false
#| fig-cap: "Estimated mean acclimation and acute $lnVR_{Q_{10}}$ for adult (a) and juvenile (j) life-history stages for marine (A), freshwater (B) and terrestrial (C) ectotherms. Across all plots, thick bars indicate 95% confidence intervals and thin bars indicate 95% prediction intervals. k = total number of effect size estimates while the numbers in brackets indicate the number of species. For ease of visualisation, raw data for both adult and juvenile life-history stages are presented but points are not distinguished by different symbols. $\\Beta$ values are the contrasts between acute and acclimation means within each life stage. $p_{MCMC}$ values are the posterior probability of the contrast being different from zero."
...
figvr_lh <-(p3_vr | p1_vr | p2_vr) + plot_annotation(tag_levels = "A") 
ggsave(figvr_lh, filename = "./output/figures/figSlh.pdf", width = 28, height = 10, device = "pdf")

figvr_lh <- magick::image_read("./output/figures/figSlh.pdf")
figvr_lh
