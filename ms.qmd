...
To better understand the characteristics of the performance curves in a sample that would lead to observed changes in variance across temperature we conducted a simple simulation. To simulate performance curves, we used a asymmetrical Gaussian function (@eq-sim):

$$
P_{T} = 2\epsilon^{-\frac{(T-\delta)^2}{2\sigma^2}}\Phi\left(\alpha\frac{T-\delta}{\sigma}\right)
$$ {#eq-sim}

where $T$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where performance is maximized), $\sigma$ the performance breadth, and $\alpha$ the skewness of the performance function or rate variation (see *Supplementary Materials* showing curves).  While we could have used other performance curve functions (e.g., Arrhenius), our intentions were not to model the exact curves (which are not know) but rather identify what might vary across the major habitat types that could contribute to the different patterns of variance change at higher temperatures we observed across habitats.
...
increase with increasing temperature across all habitat types with less heterogeneity overall ($I^2_{sv}$ = `r mean(tot_i2_VR)`, 95% CI:
`r quantile(tot_i2_VR, 0.025)` to `r quantile(tot_i2_VR, 0.975)`; "Model 1"; see *Supplementary Materials* @fig-s3). However, physiological rate variance increased significantly more in freshwater (freshwater vs terrestrial: $\beta$ = `r mean(f_t_c)`, 95% CI: `r quantile(f_t_c, 0.025)` to `r quantile(f_t_c, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(f_t_c))`) and marine ectotherms (marine vs terrestrial: $\beta$ = `r mean(m_t_c)`, 95% CI: `r quantile(m_t_c, 0.025)` to `r quantile(m_t_c, 0.975)`, $p_{MCMC}$ = `r p_value(pmcmc(m_t_c))`) compared to the limited increase in terrestrial ecotherms (@fig-2). There were no differences between
$lnVR_{Q_{10_{acute}}}$ and $lnVR_{Q_{10_{acclimation}}}$. Differences
in variance between habitats were driven by different scaling relationships between mean and variance across habitats (see *Supplementary
...
#ggsave(lnvr_sim_delta_sigma1, width = 9.5, height = 8, filename = "./output/figures/figsim_new2.pdf")

p_sim2 <- image_read("./output/figures/figsim_new.pdf")
p_sim2
```
...
([@fig-btrait]D-F; see *Supplementary Materials*, @fig-vrtrait) for more detailed trait
...
*Supplemental Material*, @fig-vrtrait) ("Model 3"). Each life-history stage
