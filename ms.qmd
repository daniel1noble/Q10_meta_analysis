...
# create all combuinations of sd
...
lncvr_sim_delta_sigma <- ggplot(sim_results, aes(x = delta, y = sigma, z = lnCVR)) +
  ...
  labs(x = TeX("Variation in Thermal Optima ($\\sigma_{\\delta}$)"),
       ...
       fill = TeX("$lnCVR_{Q_{10}}$")) + theme_classic()+ my_theme() + theme(legend.title = element_text(size = 20), legend.text = element_text(size = 12))

lnvr_sim_delta_sigma <- ggplot(sim_results, aes(x = delta, y = sigma, z = lnVR)) +
...
p_sim2 <- (lncvr_sim_delta_sigma + lnvr_sim_delta_sigma) + plot_annotation(tag_levels = "A") & theme(plot.tag = element_text(size = 30))
p_sim2
```