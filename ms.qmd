...
Analysis of $lnVR_{Q_{10}}$ suggested that variance increases with higher temperatures across all habitat types, with terrestrial ectotherms having the smallest increase in variance (@fig-figs1). 
...
```{r}
#| label: tbl-tabs1
#| tbl-cap: Model estimates, standard error, and 95% credible intervals 
...
# Headers for the table to indicate fixed and random effects
  fixed_head <- c("Fixed Effects", rep(" ", 4))
  random_head <- c("Random Effects", rep(" ", 4))
...
# Create the table
  ...
  gt(tabs1) %>%  fmt_number(
    columns = 2:5,
    decimals = 2
  ) %>% tab_row_group(label = "Fixed Effects", rows = 1:7) %>% tab_row_group(label = "Random Effects", rows = 8:11)
```
