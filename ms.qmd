...
if(rerun){
  ...
  # Have a look at the AIC of the various models to compare which power paramater is best, if any.
   delta_aic <- order(sapply(mod_list, function(x) AIC(x))) - min(order(sapply(mod_list, function(x) AIC(x))))
...
```{r, fig-1}
#| label: fig-1
#| fig-cap: "Taxonomic distribution of acute and acclimation $Q_{10}$ estimates across major habitats. **A)** Phylogenetic distribution of taxa contained within the data. The total number of acute and acclimation Q10 effect sizes are highlighted as well as whether the taxa are marine, freshwater or terrestrial. Silhouettes are representative taxa of major clades within the tree. **B)** Mean acute and acclimation $lnRR_{Q_{10}}$ across marine, freshwater, and terrestrial systems. **C)** Mean $lnCVR_{Q_{10}}$ across traits for marine, freshwater and terrestrial systems. Note there were no differences between acute and acclimation $Q_{10}$ types. k = total number of effect size estimates while the numbers in brackets indicate the number of species. Thick bars are 95% confidence intervals (CI) and thin bars 95% prediction intervals (PI)."
#| warning: false
#| echo: false
...
##################
  # Figure 1b plot
##################
  viridis_cols <- c( "#21908CFF", "#440154FF","#228C22")

  k_lnrr <- manual_extract(model1.2, mod = "habitat", group = "record_num", exclude = NULL)

p1.2 <- orchaRd::orchard_plot(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45, legend.pos = "top.left", trunk.size = 1, branch.size = 2) + scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) +
  ...
  scale_fill_manual(values = viridis_cols) + ylim(-0.5, 1.5) +  annotate("text", label= paste0("k = ", k_lnrr$k, " (", k_lnrr$sdy, ")"), x = c(0.8, 1.8, 2.8), y = 1.7, size = 5)
...
################## 
  ...
  p2 <- orchard_plot(model2.2, xlab = TeX("$lnCVR_{Q_{10}}$"), by = "type", at = list(type = c("acute", "acclim")), mod = "habitat", group  = "record_num", weights = "prop", angle = 45, trunk.size = 1, branch.size = 2, k.pos = "none") +
              scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) + ylim(-1.5, 1.5) + scale_fill_manual(values = viridis_cols) +
  ...
  annotate("text", x = x3-0.15, y = 1, 
    ...
    annotate("text", label= paste0("k = ", k_cvr$k, " (", k_cvr$sdy, ")"), x = c(0.8, 1.8, 2.8), y = 1.3, size = 5) + ylim (-2, 2)
