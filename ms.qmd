...
Projected changes in the opportunity for selection were highly variable across the globe, with some regions showing a decrease in the opportunity for selection, while others showing an increase (@fig-preds D). Out of the `r length(unique(all_pred2$match))` populations variance was predicted to decrease in `r `
