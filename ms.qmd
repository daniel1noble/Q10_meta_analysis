...
More variable populations, genetically and/or phenotypically, are
predicted to be associated with broader niches, reduced intraspecific
competition, increased growth rate, decreased vulnerability to
...
traits are linked to fitness, changes in variation can limit responses
...
variance) is log-linear (see *Supplementary Materials* for further
discussion). We test and control for any violations of these assumptions
in our analysis (see below) ([@fig-concept]B).
...
# Load and plot Figure 1. Note that this figure was derived using 3 separate code chunks described below. Figures were exported, imported into Adobe Illustrator for minor formatting changes and grid organisation. 
  fig2 <- image_read("./output/figures/Fig2_r1.png")
  plot(fig2)
...
Supplement, @fig-S2) for more detailed trait analysis). Biochemical
...
################## 
  ...
  ggsave(fig3, filename = "./output/figures/fig3_r1.png", width = 21.88, height = 9)
  
  fig3 <- magick::image_read("./output/figures/fig3_r1.png")
  fig3
```
...
([@fig-btrait]D-F; see Supplement @fig-S2) for more detailed trait
...
*Supplemental Material*, @fig-S2) ("Model 3"). Each life-history stage
...
ggsave(figbtrait, filename = "./output/figures/fig4_btrait_r1.png", width = 28, height = 14, device = "png")

figbtrait <- magick::image_read("./output/figures/fig4_btrait_r1.png")
figbtrait
```
...
ggsave(fig4, filename = "./output/figures/fig4_lh_r1.pdf", width = 28, height = 14, device = "pdf")

fig4 <- magick::image_read("./output/figures/fig4_lh_r1.pdf")
fig4
```
...
####################
  ...
  ggsave(p_cv, width = 18, height = 10, filename = "./output/figures/fig_p_cv_r1.png")

  p_cv <- image_read("./output/figures/fig_p_cv_r1.png")
  p_cv 
```
...
@Seebacher2015-wi), possibly due to the use of new $Q_{10}$ based effect
sizes that allow us to control for sampling variance. Greater capacity
for acclimation in aquatic organisms may be the result of fewer
opportunities for behavioural thermoregulation in aquatic environments
making physiological remodeling important for maintaining homeostasis
[@Gunderson2015-hz; @morley2019physiological]. While marine and
freshwater ectotherms were capable of partial acclimation it is still
important to recognise that, on average, the effect size was small
(amounting to $Q_{10}$ dropping from \~1.8 to 1.6).
...
Our results further highlight the potential vulnerability of terrestrial
...
ggsave(lnrrq10_temp, file = "./output/figures/lnrrq10_temp.png", height = 6, width = 8)

lnrrq10_temp <- magick::image_read("./output/figures/lnrrq10_temp.png")
lnrrq10_temp
...
#| fig-cap: "Bubble plot of the relationship between $lnRR_{Q_{10}}$ and acclimation time for terrestrial (green), marine (orange) and freshwater (blue) habitats. Acclimation time is centered around the mean acclimation time (37.5 days) in the data. Not all studies reported acclimation time hence the total number of effects, k, was 1767."
  ...
  ggsave(accl_time, file = "./output/figures/accl_time.png", height = 6, width = 8.5)

accl_time <- magick::image_read("./output/figures/accl_time.png")
accl_time
```
...
ggsave(fig2, filename = "./output/figures/fig2_trait.png", height = 20, width = 15)
...
ggsave(figcvrtrait, filename = "./output/figures/figcvrtrait.png", height = 20, width = 15)
...
what acclimatin and test temperature (@tbl-slopes).
...
```{r, fig-mv}
 ...
 p_mv <-  (mv1 / mv2)  + plot_annotation(tag_levels = "A") + theme(plot.tag = element_text(size = 20))
  ggsave(p_mv, file = "./output/figures/plot_mv.png", height = 8, width = 8)
  
  p_mv <- magick::image_read("./output/figures/plot_mv.png")
  p_mv
```
...
ggsave(p_acf, width = 18, height = 10, filename = "./output/figures/fig_p_acf_r1.pdf")

  p_acf <- image_read("./output/figures/fig_p_acf_r1.pdf")
  p_acf 
```
