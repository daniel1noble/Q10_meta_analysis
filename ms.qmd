...
# Lets explore the model and how it's predciting the real data
  data_w_pred <- cbind(lnCVRQ10_data_wild[complete.cases(lnCVRQ10_data_wild$c_accl_time),], predict(model5_bayes))
...
# Fitted by Lat/long
  ggplot(data = data_w_pred, aes(y = Estimate, x = lat.y)) + geom_point() + geom_smooth(method = "loess") + labs(x = "Latitude", y = "Predicted Effect Size") + theme_classic() 

  ggplot(data = data_w_pred, aes(y = Estimate, x = long.x)) + geom_point() + geom_smooth(method = "loess") + labs(x = "Longitude", y = "Predicted Effect Size") + theme_classic() 

# Observed by Lat/long
  ggplot(data = data_w_pred, aes(y = effect_size, x = lat.y)) + geom_point() + geom_smooth(method = "loess") + labs(x = "Latitude", y = "Observed Effect Size") + theme_classic() 

  ggplot(data = data_w_pred, aes(y = effect_size, x = long.x)) + geom_point() + geom_smooth(method = "loess") + labs(x = "Longitude", y = "Observed Effect Size") + theme_classic() 
...
# sigma2_v = typical sampling error variance**NEW STEP**
...
# Now check that var fix from raw data and remove sampling variance. **NEW STEP**
...
#############################
  ...
  terrest_data <- sub_data  %>% filter(habitat == "t")  %>% distinct(spp, species_full, habitat, type, lat.y, long.x)
...
## Marine ectotherms  
#############################
  marine_data <- sub_data  %>% filter(habitat == "m") %>% distinct(spp, species_full, habitat, type, lat.y, long.x)
...
#############################
  fresh_data <- sub_data  %>% filter(habitat == "f")%>% distinct(spp, species_full, habitat, type, lat.y, long.x)
...
map1_current <- ggplot() +
  ...
  geom_point(
    ...
    aes(long.x, lat.y, color = Estimate),
    alpha = 0.7, size = 5
