...
###########################################################
      # Create new data for predictions
       lh_b_data <- data.frame(     type = c("acclim", "acute"),
                              max_temp_c = 0,
                             c_accl_time = 0,
                                       V = 0,
                                 habitat = rep(c("t", "m", "f"), each = 4),
       ...
       # Predict from model. Note this is the summary.
         pred_lncvr_lh_cvr <- fitted(model2.4_bayes, 
                                     newdata = lh_b_data, 
                                     re_formula = NA,
                                     summary = TRUE, 
                                     robust = FALSE)
         pred_lncvr_lh_cvr <- cbind(lh_b_data, pred_lncvr_lh_cvr)
    
       # Predict to get the posterior. Note that we have the full posterior distribution here for each of the 12 categories in the new data. Here, we want to look at the difference between acute and acclimation within each of the categories. 
            pred_lncvr_lh_post <- fitted(model2.4_bayes, 
                                     newdata = lh_b_data, re_formula = NA,
                                     summary = FALSE, 
                                     robust = FALSE)
        contrast_lncvr_lh_post <-  pred_lncvr_lh_post[,c(1,3,5,7,9,11)] - pred_lncvr_lh_post[,c(2,4,6,8,10,12)] # Acclimation - acute.
            
        # Contrast statistics
            contrast_lncvr_lh_stats <- data.frame( habitat = rep(c("t", "m", "f"), each = 2),
                                                     trait = rep(c("tissue/organism", "biochemical"), each = 1),
                                                    diff = sapply(data.frame(contrast_lncvr_lh_post), mean),
                                                     l95 = sapply(data.frame(contrast_lncvr_lh_post), function(x) quantile(x, 0.025)),
                                                     u95 = sapply(data.frame(contrast_lncvr_lh_post), function(x) quantile(x, 0.975)),
                                                       p = sapply(data.frame(contrast_lncvr_lh_post), function(x) p_value(pmcmc(x))))
...
p1_cvr <- orchard_plot(model2.4, mod = "life.hist", at = list(c_accl_time = 0, habitat = "m"), group = "record_num", xlab = TeX("$lnCVR_{Q_{10}}$"), condition.lab = "Type", by = "type",  g = FALSE, k = FALSE, angle = 45, branch.size = branch.size, trunk.size = trunk.size) + ylim(c(-2, 2.2)) + add_theme() + ylim(-1.5, 1.1)  + annotate("text", x = x.pos, y = 0.8, label = TeX(paste0("\\it{k} = ", m_cv$k, " (", m_cv$spp, ")")), size = size_annot) + theme(legend.position = "none", axis.text.y = element_blank())
...
p3_cvr <- orchard_plot(model2.4, mod = "life.hist", at = list(c_accl_time = 0, habitat = "t"), angle = 45, group = "record_num", xlab = TeX("$lnCVR_{Q_{10}}$"), condition.lab = "Type", by = "type",  g = FALSE, k = FALSE, branch.size = branch.size, trunk.size = trunk.size)+ add_theme()  + ylim(-1.55, 1.1) + annotate("text", x = x.pos, y = 0.6, label = TeX(paste0("\\it{k} = ", t_cv$k, " (", t_cv$spp, ")")), size = size_annot) +  theme(legend.position = "none") + scale_x_discrete(labels = c("A" = "Adult","J" = "Juvenile")) 
...
# Now that we have all the predictions we can merge the data together. Note that species itself is not really that useful because each prediction at each latitude and longitude should incorporate the species effect. So we'll remove it. While I retain spp, it's still not totally specific to the species, but left it there so it's clear how these data match with the wild data. Note there are only 134 species because this is subset of the wild data.
  ...
  all_pred_no_sum <- rbind(terre_pred_no_sum, marine_pred_no_sum, fresh_pred_non_sum) %>% d
                      plyr::select(-species_full)
