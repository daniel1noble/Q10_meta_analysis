...
# Now, lets calculate the contrasts between acute a and j as well as acclim a and j within each habitat. We need to use pred for that because it's the entire posterior distribution
    # Marine 
         m_acute <-  pred[1,] - pred[3,] 
        m_acclim <-  pred[4,] - pred[2,] 
    # Freshwater
            f_ad <- 4
        f_acute <- pred[1+f_ad,] - pred[3+f_ad,] 
        f_acclim <-  pred[4+f_ad,] - pred[2+f_ad,] 
    # terrestrial
        t_ad <- 8
        t_acute <- pred[1+t_ad,] - pred[3+t_ad,] 
        t_acclim <-  pred[4+t_ad,] - pred[2+t_ad,]  

# Now lets do a quick. Note that I fixed up some design matrix issues which made acclim the ref level, which was wrong
