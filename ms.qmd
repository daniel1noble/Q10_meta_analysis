...
#### Exploring the impact of temperature on $lnRR_{Q_{10}}$ PROBABLY JUST MOVE TO THE MAIN TEXT
...
rerun = FALSE
if(rerun){
  model1.1_nl <- metafor::rma.mv(effect_size ~ max_temp_c, V = V, 
                              random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | trait,
                                            ~1 | spp,
  ...
  model1.1_bayes_nl <- brms::brm(effect_size | se(sqrt(V), sigma = TRUE) ~  max_temp_c + (~1 | record_num) + (1 | gr(species_full, cov = A)) + (1 + max_temp_c| spp) + (1|trait), data2 = list(A=A), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 8, data = lnRRQ10_data)
  
  # Save model1.1_nl
  saveRDS(model1.1_nl, file = "./output/models/lnRRQ10_1.1_nl")
  saveRDS(model1.1_bayes_nl, file = "./output/models/lnRRQ10_1.1_bayes_nl")
} else{ 
  model1.1_nl <- readRDS("./output/models/lnRRQ10_1.1_nl")
model1.1_bayes_nl <- readRDS("./output/models/lnRRQ10_1.1_bayes_nl")
}
