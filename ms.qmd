...
To better understand the characteristics of the performance curves in a sample that would lead to observed changes in variance (and relative variance) across temperature we conducted a simple simulation. To simulate the performance curves, we used a modified Gaussian function (@eq-sim):

$$
P_{z} = 2\epsilon^{-\frac{(z-\delta)^2}{2\sigma^2}}\Phi\left(\alpha\frac{z-\delta}{\sigma}\right)
$$ {#eq-sim}

where $z$ is the temperature gradient, $\delta$ is the optimal temperature (the temperature where fitness is maximised), $\sigma$ is the performance breadth, and $\alpha$ is the skewness of the fitness function or rate variation. We simulated 40 individuals with varying performance breadth, optima and rate variation. We then calculated the relative variance in fitness across the temperature gradient as the variance in fitness at each temperature divided by the maximum fitness at that temperature. The results are shown in @fig-sim.
