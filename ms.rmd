...
annotate("text", x = 3.01, y = 4.60, 
  ...
  annotate("text", x = 2.85, y = 3, label = "P = 0.36", hjust = 0, size = 6, parse = TRUE)
    
  
  p1.2 + theme_classic() + theme(axis.title  = element_text(size = 24),
        axis.text = element_text(size = 15))
  
  
   res <- mod_results(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", data = lnRRQ10_data)
 
    ggplot(lnRRQ10_data) + ggbeeswarm::geom_quasirandom(aes(y = effect_size, x = habitat, shape = type, colour = type, size = 1/sqrt(V)), alpha = 0.7)+ coord_flip() + theme_classic() + scale_x_discrete(labels = c("t" = "Terrestrial","m" = "Marine" ,"f" = "Freshwater")) + theme(axis.title.y = element_blank()) + labs(y = TeX("$lnRR_{Q_{10}}$"), size = "Precision (1/SE)",colour = "Type", shape = "Type")
    
    
    geom_quasirandomgeom_point(position = position_beeswarm(dodge.width = 0.2, groupOnX = TRUE), alpha = 0.7) 
...
```{r, fig6, fig.align='center', fig.cap= "Bubble plot environment lnRR"}
p6 <- bubble_plot(model3.5, mod = "cv_c", group  = "record_num", data = lnRRQ10_data_wild, ylab = TeX("$lnRR_{Q_{10}}$"), xlab = "Thermal Coefficient of Variation (CV)") +annotate("text", x = 1, y = 3, label = TeX(paste0("$\\beta = $", round(model3.5$b[3], 3), ", 95%CI: ", round(model3.5$ci.lb[3],3), " to ", round(model3.5$ci.ub[3], 3), " P = ", round(model3.5$pval[3],3))))

p7 <- bubble_plot(model3.6, mod = "acf_all_c", group  = "record_num", data = lnRRQ10_data_wild, ylab = TeX("$lnRR_{Q_{10}}$"), xlab = "Thermal Autocorrelation") + theme(axis.title.y = element_blank()) +annotate("text", x = -3, y = 3, label = TeX(paste0("$\\beta = $", round(model3.6$b[3], 3), ", 95%CI: ", round(model3.6$ci.lb[3],3), " to ", round(model3.6$ci.ub[3], 3), " P = ", round(model3.6$pval[3],3))))
  
p67 <- (p6 | p7) + plot_annotation(tag_levels = "A")
  
ggsave(p67, filename = "./output/figures/fig6.png", height = 10, width = 25, units = "cm")
```

```{r, fig7, fig.align='center', fig.cap= "Bubble plot environment lnCVR"}
p8 <- bubble_plot(model4.5, mod = "cv_c", group  = "record_num", data = lnRRQ10_data_wild, ylab = TeX("$lnCVR_{Q_{10}}$"), xlab = "Thermal Coefficient of Variation (CV)") +annotate("text", x = 1, y = 3, label = TeX(paste0("$\\beta = $", round(model4.5$b[3], 3), ", 95%CI: ", round(model4.5$ci.lb[3],3), " to ", round(model4.5$ci.ub[3], 3), " P = ", round(model4.5$pval[3],3))))

p8 <- bubble_plot(model3.6, mod = "acf_all_c", group  = "record_num", data = lnRRQ10_data_wild, ylab = TeX("$lnCVR_{Q_{10}}$"), xlab = "Thermal Autocorrelation") + theme(axis.title.y = element_blank()) +annotate("text", x = -3, y = 3, label = TeX(paste0("$\\beta = $", round(model4.6$b[3], 3), ", 95%CI: ", round(model4.6$ci.lb[3],3), " to ", round(model4.6$ci.ub[3], 3), " P = ", round(model4.6$pval[3],3))))
  
p89 <- (p8 | p9) + plot_annotation(tag_levels = "A")
  
ggsave(p89, filename = "./output/figures/fig7.png", height = 10, width = 25, units = "cm")
```
