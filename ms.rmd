...
# Robust variance
model1.2_rob <- robust(model1.2, cluster = lnRRQ10_data$record_num)

## Contrasts
posteriors_1.2 <- posterior_samples(model1.2_bayes, pars = "^b")
f_acute  <- posteriors_1.2[,"b_Intercept"] 
pmcmc(f_acute)
...
annotate("text", x = 1.01, y = 4.60, 
 ...
 if(rerun){
  # More realistic model with 
  model1.3 <- metafor::rma.mv(effect_size ~  c_accl_time + type + trait_category + habitat + type:habitat + trait_category:habitat:type + trait_category, V = V, 
                              random = list(~1 | record_num,
                                            ~1 | species_full,
                                            ~1 | spp,
                                            ~1 | obs),
   ...
   model1.3_bayes <- brms::brm(effect_size | se(sqrt(V)) ~  c_accl_time + type + trait_category + habitat + type:habitat + trait_category:habitat:type + trait_category + (~1 | record_num) + (1 | gr(species_full, cov = A)) + (1 | spp) + (1|obs), data2 = list(A=A), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 4, data = lnRRQ10_data)
                                  
  # Save model1.2
  saveRDS(model1.3, file = "./output/models/lnRRQ10_1.3")
  saveRDS(model1.3_bayes, file = "./output/models/lnRRQ10_1.3_bayes")

  } else{
  model1.3 <- readRDS("./output/models/lnRRQ10_1.3")
  model1.3_bayes <- readRDS("./output/models/lnRRQ10_1.3_bayes")
}

orchard_plot(model1.3, mod = "trait_category",  group = "record_num",  data = lnRRQ10_data) 

orchard_plot(model1.3, mod = "trait_category", at = list(type = c("acute", "acclim"), c_accl_time = 0, habitat = "m"), by = "type",  group = "record_num", weights = "prop", data = lnRRQ10_data, xlab = "lnRRQ10") 
```
