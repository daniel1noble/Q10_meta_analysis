...
### *Meta-Analysis*
We analysed our data using Bayesian multilevel meta-analytic (MLMA) and meta-regression (MLMR) models in R (vers. `r paste0(R.Version()$major, ".", R.Version()$minor)`) using *brms* [vers. `r utils::packageVersion("brms")` @Burkner2018; @Burkner2017; @stan]. We also refit the same models using *metafor* [vers. `r utils::packageVersion("metafor")` @Viechtbauer2010] to ensure that results were consistent, and to create orchard plots more easily [vers. 2.0, @Nakagawa2021c]. 

In all models, we included study, species and phylogeny as random effects to account for non-independence, and explore drivers of variation among $lnRR_{Q_{10}}$ and $lnCVR_{Q_{10}}$ estimates. We ran 4 MCMC chains, each with a warm up of 1000 followed by 4000 sampling iterations keeping every 5 iterations for a total of 3200 samples from the posterior distribution. We used flat Gaussian priors for 'fixed' effects (i.e., $N(0, 10)$) and a student t-distribution for 'random' effects (i.e., $student_{t}(3, 0, 10)$). We checked that all MCMC chains were effectively mixing and had converged (i.e., $R_{hat} = 1$). 
...
# Filter to different habitats
   lnRRQ10_data_m <- lnRRQ10_data %>% filter(habitat == "m") %>% mutate(type = is.character(type))
   lnRRQ10_data_f <- lnRRQ10_data %>% filter(habitat == "f") %>% mutate(type = is.character(type))
   lnRRQ10_data_t <- lnRRQ10_data %>% filter(habitat == "t") %>% mutate(type = is.character(type))
...
if(rerun){
  ...
  model1.3_m <- metafor::rma.mv(effect_size ~  c_accl_time + type + trait_category + type:trait_category, V = V, 
                              ...
                              test = "t", data = lnRRQ10_data_m %>% filter(!trait_category =="antiox" | !is.na(effect_size)))
  ...
  orchard_plot(model1.3_m_het, mod = "trait_category", at = list(type = c("acclim", "acute")), by = "type", xlab = "plot", group = "spp", data = lnRRQ10_data_m, angle = 45)
