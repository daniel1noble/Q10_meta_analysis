...
##################################
    pacman::p_load(tidyverse,  metafor, flextable, raster, rasterVis,  patchwork, RColorBrewer, ggtree, ape, phytools, viridis, MCMCglmm, brms, latex2exp, orchaRd, multcomp, ggbeeswarm, magick, ggimage)
...
Daniel W.A. Noble$^1$, Fonti Kar$^2$, Frank Seebacher$^3$, Alex Bush$^4$, & Shinichi Nakagawa$^2$
...
The final dataset included a total of `r f <- taxa_sum %>% filter(habitat == "f") %>% summarise(spp = sum(spp), k = sum(n)); f[1,1]` freshwater (fishes = `r taxa_sum %>% filter(habitat == "f" & class == "Actinopterygii") %>% summarise(sp = sum(spp))`, Molluscs = `r taxa_sum %>% filter(habitat == "f" & phylum == "Mollusca") %>% summarise(sp = sum(spp))`, Amphibians = `r taxa_sum %>% filter(habitat == "f" & class == "Amphibia") %>% summarise(sp = sum(spp))`, Reptiles = `r taxa_sum %>% filter(habitat == "f" & class == "Reptilia") %>% summarise(sp = sum(spp))`, Arthropods = `r taxa_sum %>% filter(habitat == "f" & phylum == "Arthropoda") %>% summarise(sp = sum(spp))`, and a single Crustacean and Nematode species), `r m <- taxa_sum %>% filter(habitat == "m") %>% summarise(spp = sum(spp), k = sum(n)); m[1,1]` marine (fishes = `r taxa_sum %>% filter(habitat == "m" & class %in% c("Actinopterygii", "Agnatha","Chondrichthyes")) %>% summarise(sp = sum(spp))`, Annelids = `r taxa_sum %>% filter(habitat == "m" & phylum == "Annelida") %>% summarise(sp = sum(spp))`, Molluscs = `r taxa_sum %>% filter(habitat == "m" & phylum == "Mollusca") %>% summarise(sp = sum(spp))`, Echinoderms = `r taxa_sum %>% filter(habitat == "m" & phylum == "Echinodermata") %>% summarise(sp = sum(spp))`, Reptiles = `r taxa_sum %>% filter(habitat == "m" & class == "Reptilia") %>% summarise(sp = sum(spp))`, Arthropods = `r taxa_sum %>% filter(habitat == "m" & phylum == "Arthropoda") %>% summarise(sp = sum(spp))`, and a single Crustacean and Cnidarian species), and `r t <- taxa_sum %>% filter(habitat == "t") %>% summarise(spp = sum(spp), k = sum(n)); t[1,1]` terrestrial species (Annelids = `r taxa_sum %>% filter(habitat == "t" & phylum == "Annelida") %>% summarise(sp = sum(spp))`, Molluscs = `r taxa_sum %>% filter(habitat == "t" & phylum == "Mollusca") %>% summarise(sp = sum(spp))`, Arthropods = `r taxa_sum %>% filter(habitat == "t" & phylum == "Arthropoda") %>% summarise(sp = sum(spp))`, Reptiles = `r taxa_sum %>% filter(habitat == "t" & class == "Reptilia") %>% summarise(sp = sum(spp))` and Amphibians = `r taxa_sum %>% filter(habitat == "t" & class == "Amphibia") %>% summarise(sp = sum(spp))` along with a single Tardigrade species) (Fig. \@ref(fig:fig1)A). We had more data on acute thermal responses (n = `r trimmed <- data_long2 %>% filter(grepl("lnRR", name) & !is.na(effect_size) & !is.na(V) & !V == 0); sum1 = trimmed %>% group_by(name, type) %>% summarise(n = n()) %>% filter(type =="acute"); sum(sum1$n)`) compared to thermal responses after an acclimation period (n = `r sum2 <- trimmed %>% group_by(name, type) %>% summarise(n = n()) %>% filter(type =="acclim"); sum2$n`) because both acclimation temperatures had separate acute responses (Fig. \@ref(fig:fig1)A). 

Most of the effect size estimates came from measurements of metabolic rates (both resting and maximal -- $N_{species}$ = `r mr <- traits_all %>% filter(trait_category %in% c("rest_MR", "max_MR")); sum(mr[,"spp_n"])`, $N_{effects}$ = `r sum(mr[,"k"])`, considering acute and acclimation effects together), metabolic enzymes ($N_{species}$ = `r me <- traits_all %>% filter(trait_category %in% c("metabolic_enzyme", "ATPase")); sum(me[,"spp_n"])`, $N_{effects}$ = `r me <- traits_all %>% filter(trait_category %in% c("metabolic_enzyme", "ATPase")); sum(me[,"k"])`) and whole-organism performance traits (i.e., measures of speed and endurance -- $N_{species}$ = `r per <- traits_all %>% filter(trait_category %in% c("sprint", "endurance")); sum(per[,"spp_n"])`, $N_{effects}$ = `r per <- traits_all %>% filter(trait_category %in% c("sprint", "endurance")); sum(per[,"k"])`). 
...
# Filter to different habitats
   ...
   lnRRQ10_data_t <- lnRRQ10_data %>% filter(habitat == "t")
    ...
    # Filter out antioxidant and NAs
  ...
  lnRRQ10_data_t <- lnRRQ10_data_t %>% filter(!is.na(c_accl_time))
...
```{r, wildlnRR10, echo = FALSE, message=FALSE, warning=FALSE, results='hide'}
#######################################
...
```{r wildlncvr, echo = FALSE, message=FALSE, warning=FALSE, results='hide'}
#######################################
...
```{r, fig3, fig.align='center', eval = FALSE, fig.cap="Acute and Acclimation lnRR q10 across traits for A) marine, B) freshwater and C) terrestrial systems"}

ptm <- orchard_plot(model1.3_m_het, mod = "trait_category", at = list(type = c("acute", "acclim")), by = "type", group = "record_num", weights = "prop", data = lnRRQ10_data_m, xlab = TeX("$lnRR_{Q_{10}}$"), angle = 0) +
...
ptf <- orchard_plot(model1.3_f_het, mod = "trait_category", at = list(type = c("acute", "acclim")), by = "type", group = "record_num", weights = "prop", data = lnRRQ10_data_f, xlab = TeX("$lnRR_{Q_{10}}$"), angle = 0) +
  scale_x_discrete(labels = c("Mito_leak" = "Proton Leak",
                              "Metabolic_enzyme" = "Metabolic Enzymes",
                              "Rest_MR" = "Resting Metabolic Rate",
                              "Max_MR" = "Maximum Metabolic Rate",
...
ptt <- orchard_plot(model1.3_t_het, mod = "trait_category", at = list(type = c("acute", "acclim")), by = "type", group = "record_num", weights = "prop", data = lnRRQ10_data_t, xlab = TeX("$lnRR_{Q_{10}}$"), angle = 0)
