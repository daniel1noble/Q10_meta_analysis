...
```{r acclimation_cap}
########################
### Differences in acclimation capacity between terrestrial, aquatic and marine
########################
...
rerun = FALSE
if(rerun){
  model1.1 <- metafor::rma.mv(effect_size ~ c_accl_time + type, V = V, 
...
} else{ model1.1 <- readRDS("./output/models/lnRRQ10_1.1")}

# Robust variance estimation
model1.1_rob <- robust(model1.1, cluster = lnRRQ10_data$record_num)

# OrchaRd plot of model 1.1
p1.1 <- orchaRd::orchard_plot(model1.1, mod = "type", group  = "record_num", data = lnRRQ10_data, 
                              xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45)
