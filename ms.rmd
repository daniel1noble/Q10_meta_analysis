...
if(rerun){
  ...
  model1.1_bayes <- brms::brm(effect_size | se(sqrt(V)) ~  1 + (~1 | record_num) + (1 | gr(species_full, cov = A)) + (1 | spp) + (1|obs), data2 = list(A=A), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 4, data = lnRRQ10_data)
...
# % change in acclimation
p_change_f <- ((f_acclim - f_acute) / f_acute); quantile(p_change_f, c(0.025, 0.975))
p_change_m <- ((m_acclim - m_acute) / m_acute); quantile(p_change_m, c(0.025, 0.975))
p_change_t <- ((t_acclim - t_acute) / t_acute); quantile(p_change_t, c(0.025, 0.975))

# But this is the % change on the log scale. We can also see what the Q10 change is..NEED TO BE CHECKED CAREFULLY
f_q10 <-exp((f_acclim - f_acute)-1); mean(f_q10); quantile(f_q10, c(0.025, 0.975))
m_q10 <-exp((m_acclim - m_acute)-1); mean(m_q10); quantile(m_q10, c(0.025, 0.975))
t_q10 <-exp((t_acclim - t_acute)-1); mean(t_q10); quantile(t_q10, c(0.025, 0.975))

# R2
fix <- var(predict(model1.2_bayes)[,"Estimate"])
r2_ml(model1.2)

# Orchard plot
results1.2 <- mod_results(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", data = lnRRQ10_data)

####################################################
# Model 1.3's: Trait type varied by acclimation type
# but data is seperated by habitat to simplify
####################################################
 if(rerun){
...
if(rerun){
  ...
  model2.2 <- metafor::rma.mv(effect_size ~  c_accl_time + type + habitat + type:habitat, V = V, 
   ...
   model2.2_bayes <- brms::brm(effect_size | se(sqrt(V)) ~  c_accl_time + type + habitat + type:habitat + (~1 | record_num) + (1 | gr(species_full, cov = A_CVR)) + (1 | spp) + (1|obs), data2 = list(A=A_CVR), family = gaussian(), prior = c(prior(normal(0, 10), "Intercept"), prior(student_t(3, 0, 10), "sd")), control = list(adapt_delta = 0.98, max_treedepth = 12), chains = 4, thin = 5, iter = 5000, warmup = 1000, cores = 4, data = lnCVRQ10_data)
...
p1.2 <- orchaRd::orchard_plot(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", data = lnRRQ10_data, xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45, legend.pos = "top.left") + 
...
annotate("text", x = 1.01, y = 4.60, 
         label =paste0(round((model1.2$b[3] / model1.2$b[1]*100), 2), "%"), hjust = 0) + labs(shape = "Type") +
...
annotate("text", x = 2.01, y = 4.60, 
         label =paste0(round(mean(p_change_m)*100, 2), "%"), hjust = 0) + labs(shape = "Type") +
...
annotate("text", x = 3.01, y = 4.60, 
         label =paste0(round(mean(p_change_t)*100, 2), "%"), hjust = 0) + labs(shape = "Type") +
...
```{r, fig2, fig.align='center', fig.cap="Acute and Acclimation lnRR q10 across traits for A) marine, B) freshwater and C) terrestrial systems"}

orchard_plot(model1.3, mod = "trait_category",  group = "record_num",  data = lnRRQ10_data) 

orchard_plot(model1.3_m, mod = "trait_category", at = list(type = c("acute", "acclim")), by = "type", group = "record_num", weights = "prop", data = lnRRQ10_data_m, xlab = "lnRRQ10", angle = 45) 
