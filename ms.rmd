...
# Subset the data
lnRRQ10_data <- data_long %>% filter(grepl("lnRR", name) & !is.na(effect_size) & !is.na(V) & !V == 0) %>% mutate(spp = species_full)

# Intercept only model. 
model1.1 <- metafor::rma.mv(effect_size ~ type, V = V, 
...
orchaRd::orchard_plot(model1.1, mod = "type", group  = "record_num", data = lnRRQ10_data, xlab = "lnRRQ10")
