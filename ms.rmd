...
########################
phylo_cor <- vcv(tree_p0.7, cor = TRUE)
A <- vcv(tree_p0.7, cor = FALSE) # For bayes
...
summary(glht(model1.2, linfct=rbind(c(-1,0,1,0,0,0,0), c(1,0,1,-1,0,1,0)), test=adjusted("none")))



# Orchard plot
p1.2 <- orchaRd::orchard_plot(model1.2, mod = "habitat", group  = "record_num", at = list(type = c("acute", "acclim")), by = "type", weights = "prop", data = lnRRQ10_data, xlab = TeX("$lnRR_{Q_{10}}$"), angle = 45, legend.pos = "top.left") + scale_x_discrete(labels = c("T" = "Terrestrial","M" = "Marine" ,"F" = "Freshwater")) +
