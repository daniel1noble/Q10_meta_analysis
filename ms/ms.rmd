...
To understand how climate impacts physiological acclimation abilities we used latitudes and longitudes reported in studies to extract temperature data from terrestrial and aquatic environments. Only studies on wild populations were used for climate analyses because temperature data at locations was likely reflective of past climate history. Temperature data was extracted using the ERA5 climate model. We extracted data over a 30 year period (1958-2022) for each population and species in the data. 

### *Meta-Analysis*
We analysed our data using Bayesian multilevel meta-analytic and meta-regression models in R (vers. `r paste0(R.Version()$major, ".", R.Version()$minor)`) using *brms* [vers. `r utils::packageVersion("brms")` @Burkner2018; @Burkner2017; @stan]. 
@Guangchuang2017
