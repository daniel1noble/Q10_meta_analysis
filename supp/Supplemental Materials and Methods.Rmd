...
##################################
    ...
    #data <- read_sheet(ss = "q10_newdata")
    data <- read.csv("../../data/CombinedData_wide.csv", stringsAsFactors = FALSE)
    data$lat <- as.numeric(data$lat)
    new_papers <- data[-grep("_o", data$record_num),]
    old_papers <- data[grep("_o", data$record_num),]
    ...
    # habitat
      ...
      data <- data %>%
              mutate(species_full = ifelse(species_full == "Chinemys reevesii", "Mauremys reevesii", species_full)) %>%
              mutate(species_full = ifelse(species_full == "Enithares sp", "Enithares ciliata", species_full))  %>%
...
##################################
    ...
    # #******** Problem with effect size calculation. V_lnVR and V_lnCVR are negative for a couple studies, should not be! Hence warning messages NaN in plotting of precision; explored this probalem more. It's actually incorrect data in the old dataset from Frank. Fonti explored the extraction for p088_o and error bars not found, sample sizes not correct. For second study, the N value was incorrect, should have been N = 5 not 0.11. Will exclude p_088_o from the data. Fonti did a check and tried to re-extract but in the end missing error so excluded
      ...
      #write.csv(data, file = "./output/data/data_final_wide.csv", row.names = FALSE)
##################################
...
##################################
      ...
      data_t1 <- data                                                  %>%
                 dplyr::select(c(1:26, grep("acute_t1", colnames(.)))) %>%
                 filter_if(~is.numeric(.), all_vars(!is.na(.)))        %>% # Filter all the na effect sizes 
                 rename(lnRR_Q10    = lnRR_Q10_acute_t1,       # Rename columns
                        V_lnRR_Q10  = V_lnRR_Q10_acute_t1,
                        lnVR_Q10    = lnVR_Q10_acute_t1,
                        V_lnVR_Q10  = V_lnVR_Q10_acute_t1, 
                        lnCVR_Q10   = lnCVR_Q10_acute_t1,
                        V_lnCVR_Q10 = V_lnCVR_Q10_acute_t1,
                              paper = record_num)            %>%
                 mutate(type = "acute")
      
      dim(data_t1)
      
      # Do the same for hot acute
      data_t2 <- data                                                  %>%
                 dplyr::select(c(1:26, grep("acute_t2", colnames(.)))) %>%
                 filter_if(~is.numeric(.), all_vars(!is.na(.)))        %>%  # Filter all the na effect sizes 
                 rename(lnRR_Q10    = lnRR_Q10_acute_t2,
                        V_lnRR_Q10  = V_lnRR_Q10_acute_t2,
                        lnVR_Q10    = lnVR_Q10_acute_t2,
                        V_lnVR_Q10  = V_lnVR_Q10_acute_t2, 
                        lnCVR_Q10   = lnCVR_Q10_acute_t2,
                        V_lnCVR_Q10 = V_lnCVR_Q10_acute_t2,
                              paper = record_num)            %>%
                 mutate(type = "acute")
      dim(data_t2)
      
      # Now do for the acclimation
      data_acclim <- data                                            %>%
                 dplyr::select(c(1:26, grep("acclim", colnames(.)))) %>%
                 filter_if(~is.numeric(.), all_vars(!is.na(.)))      %>% # Filter all the na effect sizes 
                 rename(lnRR_Q10    = lnRR_Q10_acclim,
                        V_lnRR_Q10  = V_lnRR_Q10_acclim,
                        lnVR_Q10    = lnVR_Q10_acclim,
                        V_lnVR_Q10  = V_lnVR_Q10_acclim, 
                        lnCVR_Q10   = lnCVR_Q10_acclim,
                        V_lnCVR_Q10 = V_lnCVR_Q10_acclim,
                              paper = record_num)            %>%
                 mutate(type = "acclim")
      dim(data_acclim)
      
      # All these data frames are exactly the same excet that the effect sizes. We've removed NA, so lets bind together
      
      data_long <- bind_rows(data_t1, data_t2, data_acclim)
...
levelplot(sst.slice ~ lon * lat, data = grid,  cuts = 11, pretty = TRUE, 
          ...
          layer(sp.points(points, pch = 16, col = "black")) + 
          layer(sp.lines(coast_outline, col="black", lwd=0.5))
...
```{r FigS1,fig.height=10, fig.cap="Global distribution of species / studies for terrestrial, freshwater and marine ectotherm acclimation research. Latitudes and longitudes of studies are projected on a global map of maximum surface temperature in A) December, B) April and C) August for world and North America" }
 ...
 p1 <- levelplot(seasonalityDec/10, margin =FALSE, cuts=20, pretty=TRUE, main = "December", col.regions=colfunc(50)) +
        layer(sp.points(points, pch = 16, col = "black")) +
        layer(sp.lines(coast_outline, col="black", lwd=0.5)) # Add coastline layer to define boundaries better +
...
if(rerun1){
  ...
  saveRDS(lnRR_Q10_mod1, "../models/lnRR_Q10_mod1")
} else{
lnRR_Q10_mod1 <- readRDS("../models/lnRR_Q10_mod1")  }
...
if(rerun2){
  ...
  saveRDS(lnRR_Q10_mod2, "../models/lnRR_Q10_mod2")
}else{
  lnRR_Q10_mod2 <- readRDS("../models/lnRR_Q10_mod2")
}
# Checks
